...
_FI_MAX_RAW_STEPS = max(1000, int(os.environ.get("COMPARE_RESULTS_FI_MAX_RAW_STEPS", "120000")))
_FI_MAX_WINDOWS = max(256, int(os.environ.get("COMPARE_RESULTS_FI_MAX_WINDOWS", "12000")))
...
print(f"[INFO] Feature importance caps: raw_steps={_FI_MAX_RAW_STEPS}, windows={_FI_MAX_WINDOWS}")
...
from core.models import CausalSmoothTCN_GRU, MultiScaleTCN_GRU, DualBranchTCN_GRU, MultiBranchTCN_GRU, FiLMConditionedTCN_GRU, BoundedResidualTCN_GRU, AffineCalibTCN_GRU, PositiveResidualTCN_GRU, AssistExpertTCN_GRU, SlowFastCalibTCN_GRU, StructuredSlowFastTCN_GRU, AdaptiveComplementaryTCN_GRU, CadenceStepLengthTCN_GRU, PersistentBiasTCN_GRU, SpeedModeResidualTCN_GRU, ObserverStateTCN_GRU, FactorizedAnchorTCN_GRU, MemoryCalibratorTCN_GRU, AssistSwitchHeadTCN_GRU, AssistOnBiasCorrectorTCN_GRU, SpeedScaleSwitchTCN_GRU, RegimeExpertTCN_GRU, AssistSplitTCN_GRU, PrivilegedKinMemoryCalibratorTCN_GRU, StrideProgressionTCN_GRU, PDRHybridTCN_GRU, PredictedKinCascadeTCN_GRU, ProgressionDistillTCN_GRU, AssistDeconfoundTCN_GRU, MultiAnchorFusionTCN_GRU, AdaptiveBiasStateTCN_GRU, MonotoneProgressionTCN_GRU, TCN_GRU_MDN
...
def slice_primary_target_np(y, primary_output_dim):
    if primary_output_dim is None:
        return y
    return y[..., :int(primary_output_dim)]
...
def _merge_level_selection(base_selection, include=None, exclude=None):
    include = list(include or [])
    exclude = list(exclude or [])
    merged = {"include": [], "exclude": []}
    if isinstance(base_selection, dict):
        merged["include"].extend(base_selection.get("include") or [])
        merged["exclude"].extend(base_selection.get("exclude") or [])
    elif base_selection:
        merged["include"].extend(base_selection)
    merged["include"].extend(include)
    merged["exclude"].extend(exclude)
    merged["include"] = list(dict.fromkeys(merged["include"]))
    merged["exclude"] = list(dict.fromkeys(merged["exclude"]))
    if not merged["include"] and not merged["exclude"]:
        return None
    return merged


def _apply_eval_level_policy(cfg, exp_path, level_selection):
    exp_name = str(exp_path).lower()
    cfg_name = str(cfg.get("exp_name", "")).lower()
    is_no_assist = "noassist" in exp_name or "no_assist" in exp_name or "noassist" in cfg_name or "no_assist" in cfg_name
    if not is_no_assist:
        return level_selection
    # Safety rail: no_assist evaluation should stay on lv0 only.
    return _merge_level_selection(level_selection, include=["lv0"], exclude=["lv4", "lv7"])
...
def _even_subsample_np(arr, max_len):
    if arr is None:
        return None
    n = int(arr.shape[0])
    if n <= max_len:
        return arr
    idx = np.linspace(0, n - 1, num=max_len, dtype=np.int64)
    return arr[idx]
...
def apply_post_processing(y_pred_raw, y_true_raw, X_list, Y_list, cfg, 
    ...
    if y_pred_raw.ndim == 3 and est_tick_ranges is None and window_output > 1:
        ...
        for i, trial_x in enumerate(X_list):
            ...
            trial_true_full = slice_primary_target_np(Y_list[i][window_size : window_size + recon_len], output_dim)
...
def _sequential_ar_inference(model, X_list, Y_list, window_size, window_output,
                             stride_eval, est_tick_ranges, ar_k, device, primary_output_dim=None):
    """
    AR 모델 전용 순차 추론.
    ...
    for X_s, Y_s in zip(X_list, Y_list):
        ...
        for t in range(0, valid_end, stride_eval):
            ...
            if tgt_idx < T:
                all_trues.append(slice_primary_target_np(Y_s[tgt_idx:tgt_idx + 1], primary_output_dim))
...
def run_tta_inference(model, loader, device, n_passes=5, noise_std=0.02, aggregation="mean", primary_output_dim=None):
    """
    ...
    with torch.no_grad():
        for batch in loader:
            xb, yb = batch[:2]
            y_trues.append(slice_primary_target_np(yb.cpu().numpy(), primary_output_dim))
...
def load_and_evaluate(exp_dir, device, return_seqs=False, return_extras=False, stride_eval_override=None):
    ...
    curr_level_selection = (
        data_cfg.get("level_selection")
        or cfg.get("03_eval", {}).get("data", {}).get("level_selection")
        or cfg.get("shared", {}).get("data", {}).get("level_selection")
        or cfg.get("level_selection")
    )
    curr_level_selection = _apply_eval_level_policy(cfg, exp_path, curr_level_selection)
    ...
    full_output_dim = Y_list[0].shape[1]
    ...
    if ar_enable:
        ...
        y_pred_raw, y_true_raw = _sequential_ar_inference(
            ...
            stride_eval, est_tick_ranges, ar_k, device, primary_output_dim=primary_output_dim
        )
    else:
        ...
        if tta_enable:
            ...
            y_pred_raw, y_true_raw = run_tta_inference(
                model, loader, device,
                n_passes=tta_n, noise_std=tta_noise, aggregation=tta_agg, primary_output_dim=primary_output_dim
            )
        else:
...
def save_feature_importance_csv(importances, feature_names, save_path):
    ...
    print(f"  [SAVED] Feature Importance CSV: {save_path}")
...
def plot_detailed_condition_trajectories(exp_path, save_dir, device,
    ...
    est_tick_ranges = data_cfg.get("est_tick_ranges") if data_cfg else cfg.get("est_tick_ranges")
    target_transform = data_cfg.get("target_transform")
    primary_output_dim = int(data_cfg.get("primary_output_dim", 1))
    ...
    if preloaded_test_sub:
        test_sub = preloaded_test_sub
    else:
        match = re.search(r"Test-((?:m\d+_)?S\d+)", folder_name)
    ...
    if not _skip_model_load:
        ...
        primary_output_dim = int(data_cfg.get("primary_output_dim", out_dim))
        out_dim = primary_output_dim
        ...
        if model_type == "StanceGatedTCN":
            ...
            model = AdaptiveComplementaryTCN_GRU(
                **common_args,
                ...
                branch_channels=model_cfg_d.get("branch_channels"),
                bias_scale=model_cfg_d.get("bias_scale", 0.18),
                residual_scale=model_cfg_d.get("residual_scale", 0.08),
                gate_temperature=model_cfg_d.get("gate_temperature", 1.0),
            )
        elif model_type == "CadenceStepLengthTCN_GRU":
            model = CadenceStepLengthTCN_GRU(
                **common_args,
                ...
                branch_input_dims=model_cfg_d.get("branch_input_dims"),
                branch_channels=model_cfg_d.get("branch_channels"),
                residual_scale=model_cfg_d.get("residual_scale", 0.05),
            )
        elif model_type == "PersistentBiasTCN_GRU":
            model = PersistentBiasTCN_GRU(
                **common_args,
                ...
                residual_scale=model_cfg_d.get("residual_scale", 0.08),
                gate_temperature=model_cfg_d.get("gate_temperature", 1.0),
                state_leak=model_cfg_d.get("state_leak", 0.92),
            )
        elif model_type == "SpeedModeResidualTCN_GRU":
            model = SpeedModeResidualTCN_GRU(
                **common_args,
                ...
                branch_channels=model_cfg_d.get("branch_channels"),
                bias_scale=model_cfg_d.get("bias_scale", 0.18),
                mode_centers=model_cfg_d.get("mode_centers", [0.75, 1.0, 1.25]),
                mode_residual_scale=model_cfg_d.get("mode_residual_scale", 0.12),
            )
        elif model_type == "ObserverStateTCN_GRU":
            model = ObserverStateTCN_GRU(
                **common_args,
                ...
                branch_input_dims=model_cfg_d.get("branch_input_dims"),
                branch_channels=model_cfg_d.get("branch_channels"),
                bias_scale=model_cfg_d.get("bias_scale", 0.12),
                residual_scale=model_cfg_d.get("residual_scale", 0.05),
                drift_scale=model_cfg_d.get("drift_scale", 0.06),
                update_stride=model_cfg_d.get("update_stride", 8),
            )
        elif model_type == "FactorizedAnchorTCN_GRU":
            model = FactorizedAnchorTCN_GRU(
                **common_args,
                ...
                branch_input_dims=model_cfg_d.get("branch_input_dims"),
                branch_channels=model_cfg_d.get("branch_channels"),
                residual_scale=model_cfg_d.get("residual_scale", 0.06),
            )
        elif model_type == "MemoryCalibratorTCN_GRU":
            model = MemoryCalibratorTCN_GRU(
                **common_args,
                ...
                branch_input_dims=model_cfg_d.get("branch_input_dims"),
                branch_channels=model_cfg_d.get("branch_channels"),
                bias_scale=model_cfg_d.get("bias_scale", 0.14),
                residual_scale=model_cfg_d.get("residual_scale", 0.04),
                calibrator_stride=model_cfg_d.get("calibrator_stride", 10),
            )
        elif model_type == "PrivilegedKinMemoryCalibratorTCN_GRU":
            model = PrivilegedKinMemoryCalibratorTCN_GRU(
                **common_args,
                ...
                residual_scale=model_cfg_d.get("residual_scale", 0.04),
                calibrator_stride=model_cfg_d.get("calibrator_stride", 10),
                aux_dim=model_cfg_d.get("aux_dim", 6),
                kin_scale=model_cfg_d.get("kin_scale", 0.04),
            )
        elif model_type == "AssistSwitchHeadTCN_GRU":
            model = AssistSwitchHeadTCN_GRU(
                **common_args,
                ...
                branch_input_dims=model_cfg_d.get("branch_input_dims"),
                branch_channels=model_cfg_d.get("branch_channels"),
                bias_scale=model_cfg_d.get("bias_scale", 0.16),
                residual_scale=model_cfg_d.get("residual_scale", 0.08),
                gate_temperature=model_cfg_d.get("gate_temperature", 1.0),
                on_scale=model_cfg_d.get("on_scale", 0.12),
                off_scale=model_cfg_d.get("off_scale", 0.05),
                hard_switch=model_cfg_d.get("hard_switch", False),
                assist_state_index=model_cfg_d.get("assist_state_index", -3),
            )
        elif model_type == "StrideProgressionTCN_GRU":
            model = StrideProgressionTCN_GRU(
                **common_args,
                ...
                branch_input_dims=model_cfg_d.get("branch_input_dims"),
                branch_channels=model_cfg_d.get("branch_channels"),
                phase_bins=model_cfg_d.get("phase_bins", 4),
                bias_scale=model_cfg_d.get("bias_scale", 0.10),
                residual_scale=model_cfg_d.get("residual_scale", 0.04),
            )
        elif model_type == "PDRHybridTCN_GRU":
            model = PDRHybridTCN_GRU(
                **common_args,
                ...
                branch_input_dims=model_cfg_d.get("branch_input_dims"),
                branch_channels=model_cfg_d.get("branch_channels"),
                pdr_scale=model_cfg_d.get("pdr_scale", 0.10),
                bias_scale=model_cfg_d.get("bias_scale", 0.10),
                residual_scale=model_cfg_d.get("residual_scale", 0.04),
            )
        elif model_type == "PredictedKinCascadeTCN_GRU":
            model = PredictedKinCascadeTCN_GRU(
                **common_args,
                ...
                aux_dim=model_cfg_d.get("aux_dim", 6),
                kin_scale=model_cfg_d.get("kin_scale", 0.04),
                kin_mix_scale=model_cfg_d.get("kin_mix_scale", 0.20),
            )
        elif model_type == "ProgressionDistillTCN_GRU":
            model = ProgressionDistillTCN_GRU(
                **common_args,
                ...
                kin_scale=model_cfg_d.get("kin_scale", 0.04),
                kin_mix_scale=model_cfg_d.get("kin_mix_scale", 0.20),
                progression_scale=model_cfg_d.get("progression_scale", 0.16),
            )
        elif model_type == "AssistDeconfoundTCN_GRU":
            model = AssistDeconfoundTCN_GRU(
                **common_args,
                ...
                branch_input_dims=model_cfg_d.get("branch_input_dims"),
                branch_channels=model_cfg_d.get("branch_channels"),
                deconfound_scale=model_cfg_d.get("deconfound_scale", 0.25),
                bias_scale=model_cfg_d.get("bias_scale", 0.10),
                residual_scale=model_cfg_d.get("residual_scale", 0.04),
            )
        elif model_type == "MultiAnchorFusionTCN_GRU":
            ...
            model = MonotoneProgressionTCN_GRU(
                **common_args,
                ...
                branch_input_dims=model_cfg_d.get("branch_input_dims"),
                branch_channels=model_cfg_d.get("branch_channels"),
                bias_scale=model_cfg_d.get("bias_scale", 0.08),
                residual_scale=model_cfg_d.get("residual_scale", 0.03),
            )
        elif model_type == "AssistOnBiasCorrectorTCN_GRU":
            model = AssistOnBiasCorrectorTCN_GRU(
                **common_args,
                ...
                residual_scale=model_cfg_d.get("residual_scale", 0.06),
                gate_temperature=model_cfg_d.get("gate_temperature", 1.0),
                on_bias_scale=model_cfg_d.get("on_bias_scale", 0.18),
                assist_state_index=model_cfg_d.get("assist_state_index", -3),
            )
        elif model_type == "SpeedScaleSwitchTCN_GRU":
            model = SpeedScaleSwitchTCN_GRU(
                **common_args,
                ...
                residual_scale=model_cfg_d.get("residual_scale", 0.06),
                gate_temperature=model_cfg_d.get("gate_temperature", 1.0),
                low_scale=model_cfg_d.get("low_scale", 0.05),
                high_scale=model_cfg_d.get("high_scale", 0.14),
                speed_temperature=model_cfg_d.get("speed_temperature", 1.0),
                hard_switch=model_cfg_d.get("hard_switch", False),
            )
        elif model_type == "RegimeExpertTCN_GRU":
            model = RegimeExpertTCN_GRU(
                **common_args,
                ...
                residual_scale=model_cfg_d.get("residual_scale", 0.06),
                gate_temperature=model_cfg_d.get("gate_temperature", 1.0),
                expert_scale=model_cfg_d.get("expert_scale", 0.14),
                assist_state_index=model_cfg_d.get("assist_state_index", -3),
                assist_high_index=model_cfg_d.get("assist_high_index", -2),
            )
        elif model_type == "AssistSplitTCN_GRU":
            model = AssistSplitTCN_GRU(
                **common_args,
    ...
    level_selection = _apply_eval_level_policy(cfg, exp_path, None)
    include_levels = set((level_selection or {}).get("include") or [])
    exclude_levels = set((level_selection or {}).get("exclude") or [])
    ...
    with h5py.File(data_path, 'r') as f:
        for cond in actual_conds:
            ...
            if include_levels:
                available_lv = [lv for lv in available_lv if lv in include_levels]
            if exclude_levels:
                available_lv = [lv for lv in available_lv if lv not in exclude_levels]
            ...
            for col_idx, lv_name in enumerate(target_lvs):
                ...
                with torch.no_grad():
                     for batch in loader_vis:
                         ...
                         targets.append(slice_primary_target_np(yb.cpu().numpy(), primary_output_dim))
...
def _run_trial_prediction(model, X_arr, Y_arr, cfg, window_size, window_output,
    ...
    data_cfg_local = cfg.get("02_train", {}).get("data", cfg.get("data", {}))
    primary_output_dim = int(data_cfg_local.get("primary_output_dim", 1))
    X_eval = X_arr
    if mean is not None:
        try:
...
def save_regime_metrics_for_experiment(exp_path, output_dir, device,
    ...
    with h5py.File(data_path, 'r') as f:
        ...
        for cond in actual_conds:
            level_names = sorted(list(f[real_sub][cond].keys()))
            if include_levels:
                level_names = [lv for lv in level_names if lv in include_levels]
            if exclude_levels:
                level_names = [lv for lv in level_names if lv not in exclude_levels]
            for lv_name in level_names:
                ...
                for X_arr, Y_arr, meta in zip(X_list, Y_list, meta_list):
                    ...
                    if t_seq is None:
                        continue
                    scatter_key = (cond, lv_name)
                    if scatter_key not in scatter_groups:
                        scatter_groups[scatter_key] = {"true": [], "pred": []}
                    scatter_groups[scatter_key]["true"].append(np.asarray(t_seq, dtype=np.float32).reshape(-1))
                    scatter_groups[scatter_key]["pred"].append(np.asarray(p_seq, dtype=np.float32).reshape(-1))
                    rows.append({
                        "model": folder_name,
                        "subject": test_sub,
                        "condition": cond,
    ...
    try:
        cond_labels = summary_df.apply(lambda r: f"{r['condition']}\n{r['level']}", axis=1)
        plt.figure(figsize=(max(10, len(summary_df) * 0.7), 5))
        colors = ["#2d6a4f" if v >= 0 else "#c1121f" for v in summary_df["r2"].tolist()]
        plt.bar(range(len(summary_df)), summary_df["r2"], color=colors)
        plt.xticks(range(len(summary_df)), cond_labels, rotation=45, ha="right")
        plt.ylabel("R²")
        plt.title(f"Condition-wise R² ({folder_name})")
        plt.axhline(0.0, color="black", linewidth=1.0, alpha=0.7)
        plt.tight_layout()
        plt.savefig(output_dir / "condition_r2.png", dpi=150)
        plt.close()
    except Exception as e:
        print(f"[WARN] Failed to save condition R² plot for {folder_name}: {e}")

    try:
        n_panels = len(scatter_groups)
        if n_panels > 0:
            n_cols = 3
            ...
            for ax_idx, ((cond, level), seqs) in enumerate(sorted(scatter_groups.items())):
                ax = axes[ax_idx]
                y_true = np.concatenate(seqs["true"]).astype(np.float32)
                y_pred = np.concatenate(seqs["pred"]).astype(np.float32)
                ...
                if y_true.size > max_points:
                    idx = np.linspace(0, y_true.size - 1, num=max_points, dtype=np.int64)
                ...
                ax.scatter(y_pred_plot, y_true_plot, s=8, alpha=0.28, color="#1d3557", edgecolors="none")

                lo = float(min(np.min(y_pred_plot), np.min(y_true_plot)))
                hi = float(max(np.max(y_pred_plot), np.max(y_true_plot)))
                if np.isfinite(lo) and np.isfinite(hi):
                    ax.plot([lo, hi], [lo, hi], linestyle="--", color="#6c757d", linewidth=1.0, alpha=0.9)
                    if y_pred.size >= 2 and np.std(y_pred) > 1e-8:
                        slope, intercept = np.polyfit(y_pred, y_true, deg=1)
                        xs = np.linspace(lo, hi, 100)
                        ys = slope * xs + intercept
                        ax.plot(xs, ys, color="#d62828", linewidth=1.5)
                    ax.set_xlim(lo, hi)
                    ax.set_ylim(lo, hi)

                ax.set_title(f"{cond}\n{level}")
                ax.set_xlabel("Estimated speed [m/s]")
            ...
            fig.suptitle(f"Condition-wise Estimated vs True Speed ({folder_name})", y=0.995, fontsize=14)
            fig.tight_layout()
            fig.savefig(output_dir / "condition_r2_scatter.png", dpi=160, bbox_inches="tight")
            plt.close(fig)
    except Exception as e:
        print(f"[WARN] Failed to save condition R² scatter plot for {folder_name}: {e}")
...
def render_full_outputs_for_result(exp_path, fold_output_dir, res, skip_feature_importance=False):
    ...
    plot_detailed_condition_trajectories(
        ...
        preloaded_test_sub=res.get("test_sub"),
    )
    ...
    if skip_feature_importance:
        ...
        return
    fi_png_path = fold_output_dir / "feature_importance.png"
    fi_csv_path = fold_output_dir / "feature_importance.csv"
    if fi_png_path.exists() and fi_csv_path.exists():
    ...
    import h5py
    actual_conds = []
    fi_level_selection = _apply_eval_level_policy(_fi_cfg, exp_path, None)
    fi_include_levels = list((fi_level_selection or {}).get("include") or [])
    with h5py.File(data_path, 'r') as f_check:
        ...
        for cond in actual_conds:
            ...
            try:
                x_list, y_list = extract_condition_data_v2(
                    ...
                    lpf_cutoff=eval_lpf, lpf_order=curr_lpf_order, fs=100,
                    include_levels=fi_include_levels if fi_include_levels else None,
                )
    ...
    fi_window = int(_fi_data_cfg.get("window_size", 200))
    fi_est_ticks = _fi_data_cfg.get("est_tick_ranges", [5])
    fi_output_window = len(fi_est_ticks)
    base_stride = max(1, int(_fi_data_cfg.get("stride_inf", 1)))
    ...
    raw_caps = [
        int(_FI_MAX_RAW_STEPS),
        min(int(_FI_MAX_RAW_STEPS), 60000),
        min(int(_FI_MAX_RAW_STEPS), 30000),
        min(int(_FI_MAX_RAW_STEPS), 15000),
        min(int(_FI_MAX_RAW_STEPS), 8000),
        min(int(_FI_MAX_RAW_STEPS), 4000),
        min(int(_FI_MAX_RAW_STEPS), 2000),
    ]
    raw_caps = [cap for i, cap in enumerate(raw_caps) if cap >= 1000 and cap not in raw_caps[:i]]

    window_caps = [
        int(_FI_MAX_WINDOWS),
        min(int(_FI_MAX_WINDOWS), 3000),
        min(int(_FI_MAX_WINDOWS), 1500),
        min(int(_FI_MAX_WINDOWS), 800),
        min(int(_FI_MAX_WINDOWS), 400),
        256,
    ]
    window_caps = [cap for i, cap in enumerate(window_caps) if cap >= 64 and cap not in window_caps[:i]]

    batch_caps = []
    curr_bs = int(_FI_BATCH_SIZE)
    while curr_bs >= 1:
        if curr_bs not in batch_caps:
            batch_caps.append(curr_bs)
        if curr_bs == 1:
            break
    ...
    for raw_cap in raw_caps:
        X_try = X_test
        Y_try = Y_test
        if X_try.shape[0] > raw_cap:
            X_try = _even_subsample_np(X_try, raw_cap)
            Y_try = _even_subsample_np(Y_try, raw_cap)
        max_possible_windows = max(1, X_try.shape[0] - fi_window - max(fi_est_ticks) + 1)
        for window_cap in window_caps:
            fi_stride = max(base_stride, int(np.ceil(max_possible_windows / float(window_cap))))
            imp_ds = LazyWindowDataset(
                ...
                fi_stride,
                target_mode="sequence",
                est_tick_ranges=fi_est_ticks,
            )
            for batch_cap in batch_caps:
                attempt_idx += 1
                print(
                    f"    [FI attempt {attempt_idx}] raw_cap={raw_cap}, window_cap={window_cap}, "
                    f"stride={fi_stride}, batch={batch_cap}"
                )
                imp_loader = DataLoader(imp_ds, batch_size=batch_cap, shuffle=False, num_workers=0)
                try:
                    imps = calculate_permutation_importance(
                        _fi_model, imp_loader, device=device, criterion=criterion, batch_size=batch_cap
                    )
                    if imps.size > 0:
                        print(f"    [FI success] attempts={attempt_idx}")
                        break
                    last_error = RuntimeError("Permutation importance returned empty result")
                except (RuntimeError, MemoryError) as e:
                    last_error = e
                    print(f"    [FI retry] {type(e).__name__}: {e}")
                finally:
                    try:
                        del imp_loader
                    except Exception:
    ...
    if imps.size == 0:
        print(f"    [ERROR] Feature importance failed after {attempt_idx} attempts: {last_error}")
        return

    plot_feature_importance(imps, f_names, fi_png_path, title=f"Importance ({loss_name}): {fold_name}")
    save_feature_importance_csv(imps, f_names, fi_csv_path)
...
def analyze_detailed_single(exp_path):
    ...
    detail_level_selection = (
        cfg.get("02_train", {}).get("data", {}).get("level_selection")
        or cfg.get("03_eval", {}).get("data", {}).get("level_selection")
        or cfg.get("shared", {}).get("data", {}).get("level_selection")
        or cfg.get("level_selection")
    )
    detail_level_selection = _apply_eval_level_policy(cfg, exp_path, detail_level_selection)
    X, Y = build_nn_dataset_multi(
        cfg, [test_sub], actual_conds, c_in_vars, c_out_vars, 
        window_size, window_output, data_stride,
        condition_selection=CONDITION_SELECTION, level_selection=detail_level_selection,
        lpf_cutoff=eval_lpf, 
        lpf_order=curr_lpf_order,
        target_lpf_mode=curr_target_lpf_mode,
        est_tick_ranges=est_tick_ranges
    ...
    model_type = model_cfg.get("type", "TCN")
    primary_output_dim = int(data_cfg.get("primary_output_dim", Y[0].shape[1]))
    common_args = dict(
        input_dim=input_dim,
        output_dim=primary_output_dim,
        horizon=horizon,
    ...
    if X_imp.shape[0] > _FI_MAX_RAW_STEPS:
        orig_len = int(X_imp.shape[0])
        X_imp = _even_subsample_np(X_imp, _FI_MAX_RAW_STEPS)
        Y_imp = _even_subsample_np(Y_imp, _FI_MAX_RAW_STEPS)
        print(f"[INFO] FI raw steps downsampled: {orig_len} -> {X_imp.shape[0]}")
    ...
    try:
        fi_window = int(
            cfg.get("time_window_input")
            or cfg.get("03_eval", {}).get("data", {}).get("window_size")
            or cfg.get("02_train", {}).get("data", {}).get("window_size")
            or cfg.get("shared", {}).get("data", {}).get("window_size")
            or 100
        )
        fi_output_window = int(
            cfg.get("time_window_output")
            or cfg.get("03_eval", {}).get("data", {}).get("time_window_output")
            or cfg.get("02_train", {}).get("data", {}).get("time_window_output")
            or cfg.get("shared", {}).get("data", {}).get("time_window_output")
            or 1
        )
        base_stride = max(
            1,
            int(
                cfg.get("03_eval", {}).get("data", {}).get("stride_inf")
                or cfg.get("02_train", {}).get("data", {}).get("stride_inf")
                or cfg.get("shared", {}).get("data", {}).get("stride_inf")
                or 1
            )
        )
        max_possible_windows = max(1, X_imp.shape[0] - fi_window - max(curr_est_tick_ranges) + 1)
        fi_stride = max(base_stride, int(np.ceil(max_possible_windows / float(_FI_MAX_WINDOWS))))
        print(f"[INFO] FI dataset settings: window={fi_window}, out={fi_output_window}, stride={fi_stride}, est_ticks={curr_est_tick_ranges}")
        ds = LazyWindowDataset(
            [X_imp],
            [Y_imp],
            fi_window,
            ...
            fi_stride,
            target_mode="sequence",
            est_tick_ranges=curr_est_tick_ranges,
        )
...
def calculate_permutation_importance(model, loader, device, criterion, batch_size=None):
    ...
    N, T, D = X_cpu.shape
    if N > _FI_MAX_WINDOWS:
        idx = torch.linspace(0, N - 1, steps=_FI_MAX_WINDOWS, dtype=torch.long)
        X_cpu = X_cpu[idx]
        Y_cpu = Y_cpu[idx]
        N = X_cpu.shape[0]
        print(f"  [INFO] FI window downsampled to {N} windows")
