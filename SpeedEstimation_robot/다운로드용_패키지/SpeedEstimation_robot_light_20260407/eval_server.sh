...
echo "[INFO] Running compare_results.py on $(hostname) with CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-unset}"
echo "[INFO] Arguments: $@"
...
detail_batch_size="${EVAL_DETAIL_BATCH_SIZE:-128}"
fi_batch_size="${EVAL_FI_BATCH_SIZE:-128}"
...
if [ "$workers" -eq 1 ]; then
    ...
    echo "[INFO] Single-worker eval. threads=${threads_per_worker}, ram_limit_gb=${ram_per_worker_gb}, skip_fi=${skip_feature_importance}, fast_eval=${fast_eval}, fast_stride=${fast_eval_stride}, topk=${full_plot_topk}, eval_bs=${eval_batch_size}, detail_bs=${detail_batch_size}, fi_bs=${fi_batch_size}"
...
echo "[INFO] Parallel eval on one GPU. workers=${workers}, threads_per_worker=${threads_per_worker}, ram_limit_gb=${ram_per_worker_gb}, skip_fi=${skip_feature_importance}, fast_eval=${fast_eval}, fast_stride=${fast_eval_stride}, topk=${full_plot_topk}, eval_bs=${eval_batch_size}, detail_bs=${detail_batch_size}, fi_bs=${fi_batch_size}"
...
if [ "$fast_eval" = "1" ] && [ "$full_plot_topk" -gt 0 ]; then
    echo "[INFO] Fast eval shards finished. Rendering full plots for top ${full_plot_topk} experiments."
    topk_args=(--render-topk-only --full-plot-topk "$full_plot_topk")
    if [ "$skip_feature_importance" = "1" ]; then
        topk_args+=(--skip-feature-importance)
    fi
    COMPARE_RESULTS_THREADS="${SLURM_CPUS_PER_TASK:-8}" \
    COMPARE_RESULTS_RAM_GB=15 \
    COMPARE_RESULTS_EVAL_BATCH_SIZE="$eval_batch_size" \
    COMPARE_RESULTS_DETAIL_BATCH_SIZE="$detail_batch_size" \
    COMPARE_RESULTS_FI_BATCH_SIZE="$fi_batch_size" \
