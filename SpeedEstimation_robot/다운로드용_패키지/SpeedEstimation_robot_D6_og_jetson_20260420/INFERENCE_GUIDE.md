...
- treadmill belt speed: `speed_leftbelt`, `speed_rightbelt`
- subject height

No GRF, mocap, assist metadata, or future samples are used.

## 4. TVCF Tilt Calibration

The D6 OG model uses a time-varying complementary filter to estimate online trunk tilt.
...
### TVCF parameters used for D6 OG
...
### 5.2 Overground compensation

The OG variant applies an additional lateral belt acceleration correction:

```text
acc_hy = acc_hy + d/dt((speed_leftbelt + speed_rightbelt) / 2)
```

This aligns the tilt-corrected lateral channel with overground acceleration.

### 5.3 Horizontal velocity channels
...
- leaky integrator via `compute_imu_integrated_speed`
- causal derivative for belt speed

### 5.4 Magnitude channels
