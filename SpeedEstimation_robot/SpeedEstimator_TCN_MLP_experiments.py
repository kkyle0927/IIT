...
from data.loader import extract_condition_data_v2, build_nn_dataset, build_nn_dataset_multi, _get_feature_group
...
if __name__ == "__main__":
    ...
    # Run Distribution Analysis at the very beginning (opt-in)
    ...
    for exp_name, cfg in exp_iter:
        ...
        for sd in seeds:
            ...
            if 'data_path' not in cfg:
                ...
                if not src: src = "./combined_data_S008.h5"
                cfg['data_path'] = src
            ...
            if curr_lpf_cutoff is None and '02_train' in cfg and 'data' in cfg['02_train']:
                curr_lpf_cutoff = cfg['02_train']['data'].get('lpf_cutoff')
            ...
            if curr_lpf_order is None and '02_train' in cfg and 'data' in cfg['02_train']:
                curr_lpf_order = cfg['02_train']['data'].get('lpf_order')
            ...
            if curr_target_lpf_mode is None:
                curr_target_lpf_mode = base_config.get("target_lpf_mode", "zero_phase") if base_config else "zero_phase"
            ...
            # Per-group LPF config (archive_round8+)
            ...
            # Per-group noise augmentation config (archive_round8+)
            curr_noise_per_group = None
            if '02_train' in cfg and 'train' in cfg['02_train']:
                curr_noise_per_group = cfg['02_train']['train'].get('augment_noise_per_group')
            if curr_noise_per_group is None and 'shared' in cfg:
                curr_noise_per_group = cfg['shared'].get('augment_noise_per_group')
            ...
            for run_meta in sub_runs:
                ...
                curr_level_selection = (
                    data_cfg_now.get("level_selection")
                ...
                X_train, Y_train = build_nn_dataset_multi(
                    ...
                    condition_selection=CONDITION_SELECTION, level_selection=curr_level_selection,
                    lpf_cutoff=curr_lpf_cutoff, lpf_order=curr_lpf_order,
                ...
                # Build per-dim noise vector if augment_noise_per_group is set
                if curr_noise_per_group is not None:
                    noise_per_dim = []
                    for gp, vs in c_in_vars:
                        for v in vs:
                            fgroup = _get_feature_group(gp, v)
                            noise_per_dim.append(curr_noise_per_group.get(fgroup, 0.0))
                    cfg.setdefault('02_train', {}).setdefault('train', {})['noise_std_per_dim'] = noise_per_dim
                    print(f"[INFO] Per-group noise augmentation: {curr_noise_per_group}")
    ...
    if not args.run_feature_importance:
        print("\n[INFO] Skipping feature importance (use --run_feature_importance to enable)")
        sys.exit(0)
    ...
    best_target_lpf_mode = (
        best_cfg.get("target_lpf_mode")
        or best_cfg.get("shared", {}).get("target_lpf_mode")
        or best_cfg.get("02_train", {}).get("data", {}).get("target_lpf_mode")
        or best_cfg.get("01_construction", {}).get("target_lpf_mode")
        or "zero_phase"
    )

    best_window = best_cfg.get("time_window_input") or best_cfg.get("shared", {}).get("data", {}).get("window_size")
    best_level_selection = (
        best_cfg.get("02_train", {}).get("data", {}).get("level_selection")
        or best_cfg.get("03_eval", {}).get("data", {}).get("level_selection")
        or best_cfg.get("shared", {}).get("data", {}).get("level_selection")
        or best_cfg.get("level_selection")
    )
    ...
    X_test_best, Y_test_best = build_nn_dataset_multi(
        ...
        condition_selection=CONDITION_SELECTION, level_selection=best_level_selection,
        lpf_cutoff=best_lpf_cutoff, lpf_order=best_lpf_order,
        target_lpf_mode=best_target_lpf_mode,
        est_tick_ranges=best_cfg.get("est_tick_ranges", None)
    )

    _tw_out = best_cfg.get("time_window_output", 1)
    _est_ticks = best_cfg.get("est_tick_ranges")
    if _tw_out > 1:
        best_horizon = _tw_out
    elif _est_ticks and len(_est_ticks) > 1:
        best_horizon = len(_est_ticks)
    else:
        best_horizon = 1
