...
def _causal_running_mean_np(y, window):
    window = max(1, int(window))
    if window <= 1:
        return y.copy()
    out = np.zeros_like(y, dtype=np.float32)
    csum = np.cumsum(y, axis=0, dtype=np.float64)
...
class LazyWindowDataset(Dataset):
    def __init__(self, X_list, Y_list, input_window, output_window, stride, target_mode="sequence", est_tick_ranges=None, augment=False, noise_std=0.0, ar_k=0, device=None, time_warp_sigma=0.0, noise_std_per_dim=None, target_transform=None, input_mean=None, input_std=None, extra_targets=None):
        """
        ...
        self.Y_lf_list = None
        self.Y_hf_list = None
        self.Y_abs_dy_list = None
        self.Y_aux_tail_list = None
        lf_cfg = self.extra_targets.get("lf_hf") if isinstance(self.extra_targets, dict) else None
        aux_tail_cfg = self.extra_targets.get("aux_tail") if isinstance(self.extra_targets, dict) else None
        if lf_cfg:
            lf_window = int(lf_cfg.get("window", 101))
            self.Y_lf_list = []
            self.Y_hf_list = []
            self.Y_abs_dy_list = []
            for y in Y_list:
                y_np = np.asarray(y, dtype=np.float32)
                y_lf = _causal_running_mean_np(y_np, lf_window)
                y_hf = (y_np - y_lf).astype(np.float32)
                y_abs_dy = np.zeros_like(y_np, dtype=np.float32)
                y_abs_dy[1:] = np.abs(y_np[1:] - y_np[:-1])
                y_lf_t = torch.from_numpy(y_lf).float()
                y_hf_t = torch.from_numpy(y_hf).float()
                y_abs_dy_t = torch.from_numpy(y_abs_dy).float()
                if device is not None:
                    y_lf_t = y_lf_t.to(device)
                    y_hf_t = y_hf_t.to(device)
                    y_abs_dy_t = y_abs_dy_t.to(device)
                self.Y_lf_list.append(y_lf_t)
                self.Y_hf_list.append(y_hf_t)
                self.Y_abs_dy_list.append(y_abs_dy_t)
        if aux_tail_cfg:
            start_dim = int(aux_tail_cfg.get("start_dim", 1))
            end_dim = aux_tail_cfg.get("end_dim")
            self.Y_aux_tail_list = []
            for y in Y_list:
                y_np = np.asarray(y, dtype=np.float32)
                tail_np = y_np[:, start_dim:end_dim] if end_dim is not None else y_np[:, start_dim:]
                tail_t = torch.from_numpy(tail_np).float()
                if device is not None:
                    tail_t = tail_t.to(device)
                self.Y_aux_tail_list.append(tail_t)
    ...
    def __getitem__(self, idx):
        ...
        extra = None
        if self.Y_lf_list is not None:
            if self.est_tick_ranges:
                extra = {
                    "target_lf": self.Y_lf_list[s_idx][y_indices],
                    "target_hf": self.Y_hf_list[s_idx][y_indices],
                    "target_abs_dy": self.Y_abs_dy_list[s_idx][y_indices],
                }
            else:
                extra = {
                    "target_lf": self.Y_lf_list[s_idx][limit:end],
                    "target_hf": self.Y_hf_list[s_idx][limit:end],
                    "target_abs_dy": self.Y_abs_dy_list[s_idx][limit:end],
                }
            if self.target_mode == "mean":
                extra = {k: v.mean(dim=0) for k, v in extra.items()}
            elif self.target_mode == "last":
                extra = {k: v[-1, :] for k, v in extra.items()}
        if self.Y_aux_tail_list is not None:
            if self.est_tick_ranges:
                aux_tail = self.Y_aux_tail_list[s_idx][y_indices]
            else:
                aux_tail = self.Y_aux_tail_list[s_idx][limit:end]
            if self.target_mode == "mean":
                aux_tail = aux_tail.mean(dim=0)
            elif self.target_mode == "last":
                aux_tail = aux_tail[-1, :]
            if extra is None:
                extra = {}
            extra["aux_tail"] = aux_tail
