...
- GPU 4개 모두 사용 가능. 단, 다른 사용자가 점유 중인 GPU에는 작업을 올리지 말 것
...
├── core/
│   ├── models.py          # TCN_MLP, TCN_GRU_Head, SlowFast/StructuredSlowFast, Observer/Factorized/MemoryCalibrator 등 시계열 모델
...
- `derived/body_frame_imu_zonlypca`, `derived/body_frame_imu_zonlypcagated`: stationary 단계에서는 `+Z`만 gravity로 맞추고, walking 중에는 PCA 기반 heading으로 `+Y`를 추정한 body-frame IMU
- `derived/gravity_aligned_imu_zonly`: `+Z`만 gravity로 맞춘 뒤 horizontal 2축을 그대로 유지한 2D IMU feature (`acc_hx`, `acc_hy`, `acc_up`, `gyro_hx`, `gyro_hy`, `gyro_up`)
- `derived/tilt_local_imu_2d`: yaw는 유지하고 roll/pitch만 제거한 local 2D IMU feature (`acc_hx`, `acc_hy`, `acc_up`, `gyro_hx`, `gyro_hy`, `gyro_up`)
- `derived/tilt_cf_imu_2d`, `derived/tilt_cf_imu_2d_slow`: complementary filter로 실시간 `roll/pitch`를 추정해 tilt-only local 2D IMU를 만드는 feature (`acc_hx`, `acc_hy`, `acc_up`, `gyro_hx`, `gyro_hy`, `gyro_up`, `roll_cf`, `pitch_cf`)
- `derived/imu_integrated_speed`: forward_accel_overground의 leaky integration
- `derived/imu_integrated_speed_zonlypca`, `derived/imu_integrated_speed_zonlypcagated`: z-only + PCA heading body-frame에서 만든 integrated speed
- `derived/imu_horizontal_velocity_zonly`: gravity-aligned horizontal 2축 가속도를 각각 leaky integration한 velocity proxy (`vel_hx`, `vel_hy`)
- `derived/tilt_local_velocity_2d`: tilt-only local horizontal 2축 가속도를 각각 leaky integration한 velocity proxy (`vel_hx`, `vel_hy`)
- `derived/tilt_cf_velocity_2d`, `derived/tilt_cf_velocity_2d_slow`: complementary-filter tilt-local horizontal 2축 가속도를 각각 leaky integration한 velocity proxy (`vel_hx`, `vel_hy`)
- `derived/tilt_local_summary_2d`: tilt-only local 2D IMU와 velocity에서 만든 compact magnitude feature (`acc_h_mag`, `gyro_h_mag`, `vel_h_mag`)
- `derived/tilt_cf_summary_2d`, `derived/tilt_cf_summary_2d_slow`: complementary-filter tilt-local 2D IMU와 velocity에서 만든 compact magnitude feature (`acc_h_mag`, `gyro_h_mag`, `vel_h_mag`)
- `derived/treadmill_acc_scalar`: treadmill belt speed의 causal derivative로 만든 belt acceleration scalar (`treadmill_acc`)
...
- `derived/robot_biomech_rt`: anchor disagreement/confidence, assist on/off elapsed time, assist duty cycle, cumulative/normalized work, trunk-joint coherence, excursion residual 같은 causal plateau-calibration feature를 포함할 수 있다
- `derived/robot_biomech_rt`의 internal IMU anchor는 `derived/imu_integrated_speed`와 같은 정의를 따를 것. 즉 `body-frame forward accel + treadmill accel`을 사용해 `imu_anchor_speed_rt`를 계산하고, 여기서 `anchor_confidence`, `plateau_confidence`, `anchor_consensus_speed`, `anchor_consensus_spread`, `highspeed_anchor_hint`를 파생할 것
- `derived/wo_priors_rt`: robot+trunk-only로 계산한 complementary-filter orientation/velocity, stride-speed prior, interaction work/power, contact-probability proxy를 포함할 수 있다
...
- training-time privileged supervision으로 `mocap/kin_q`를 auxiliary target이나 distillation target으로 사용하는 것은 허용한다. 이 값들은 deploy/inference 입력으로 연결하지 말 것
