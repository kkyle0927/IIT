...
if [ -z "$SLURM_JOB_ID" ]; then
    ...
    confirm_and_submit() {
        derive_archive_job_prefix() {
            local cfg_path="$1"
            local season_num=""
            local archive_num=""

            if [[ "$cfg_path" =~ archive_season([0-9]+) ]]; then
                season_num="${BASH_REMATCH[1]}"
            fi
            if [[ "$cfg_path" =~ archive_[^/]*([0-9]+) ]]; then
                archive_num="${BASH_REMATCH[1]}"
            fi

            if [ -n "$season_num" ] && [ -n "$archive_num" ]; then
                echo "Speed_wo_grf_AS${season_num}_A${archive_num}"
            elif [ -n "$season_num" ]; then
                echo "Speed_wo_grf_AS${season_num}_AX"
            else
                echo "Speed_wo_grf_MISC"
            fi
        }
        ...
        echo "  GPU 내 worker : 기본 3개, util-based 증감 끔, OOM 시 3→2→1"
        ...
        if command -v whiptail &>/dev/null; then
            whiptail --title "제출 확인" \
                --yesno "${num_launchers}개 launcher job을 제출하시겠습니까?\n\n동시 GPU: ${MAX_PARALLEL} | GPU 내 worker: 기본 3개, util-based 증감 끔, OOM 시 3→2→1" \
        ...
        for ((slot=0; slot<num_launchers; slot++)); do
            ...
            local job_prefix
            local job_name
            job_prefix=$(derive_archive_job_prefix "${launcher_cfgs[0]}")
            job_name="${job_prefix}_train_$((slot+1))"
...
echo "[SCHED] Same-GPU workers: default=${default_target}, current=${current_target}, range=${TRAIN_MIN_WORKERS}-${TRAIN_MAX_WORKERS}, retries=${TRAIN_MAX_RETRIES}, util_adaptive=${TRAIN_ADAPTIVE_WORKERS}"
    ...
    for i in "${!active_pids[@]}"; do
        ...
        if [ "$status" -eq 0 ]; then
            ...
            if [ "$current_target" -lt "$default_target" ]; then
                ...
                echo "[SCHED] Successful completion detected after backoff. Restoring target workers to ${current_target}."
            fi
            continue
        fi
