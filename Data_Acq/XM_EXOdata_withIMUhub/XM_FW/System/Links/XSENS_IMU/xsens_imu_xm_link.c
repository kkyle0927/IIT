...
#define IMU_PACKET_TIMEOUT_MS   (1000) // 1000ms간 패킷이 없으면 연결 끊김 간주 (matches IMU_Hub pattern)
...
void XsensIMU_XM_Link_UpdateData(const XsensMTi_packet_t* packet)
{
    ...
    s_imu_xm_link.lastPacketTimeMs = IOIF_TIM_GetTick();
...
static void _Link_RunPeriodic(void)
{
    ...
    if (_Link_IsConnected()) {
        ...
        if (IOIF_TIM_GetTick() - s_imu_xm_link.lastPacketTimeMs > IMU_PACKET_TIMEOUT_MS) {
