 ...
 */
typedef struct {
    CM_RxData_t buffers[2];      /**< Ping-Pong 버퍼 */
    volatile uint8_t write_idx;  /**< ISR이 쓰는 버퍼 인덱스 (0 or 1) */
} CM_DoubleBuffer_t;

static CM_DoubleBuffer_t s_cm_double_buf = {
    .buffers = {{0}, {0}},
    .write_idx = 0
};
...
static void _CM_DecodeRxPDOData(CM_RxData_t* dst_buf);
...
void CM_ProcessCANMessage(uint16_t canId, uint8_t* data, uint8_t len)
{
    ...
    switch (fncCode) {
        case SDO:
            /* ✅ SDO는 Task Context에서 처리 (Queue를 통해 전달됨) */
            /* Lock 불필요: ISR이 아닌 Task에서 실행됨 */
            _CM_UnpackSDO(data, len);
            break;
        // PDO data는 CM_UpdatePdoData에서 ISR 컨텍스트로 직접 처리
...
/**
 * @brief ISR 컨텍스트에서 호출되어 PDO 데이터를 Double Buffer에 저장합니다.
 * @details ✅ Lock-Free: ISR에서 write_idx 버퍼에 직접 쓰고 Atomic Swap
 * @param[in] data    수신된 데이터 버퍼 포인터.
 * @param[in] len     수신된 데이터의 길이 (바이트).
 */
void CM_UpdatePdoData(uint8_t* data, uint8_t len)
{
    /* ✅ Lock-Free Double Buffer: ISR에서 write_idx 버퍼에 직접 쓰기 */
    uint8_t write_idx = s_cm_double_buf.write_idx;
    CM_RxData_t* write_buf = &s_cm_double_buf.buffers[write_idx];
    
    /* PDO Unpack & Decode (Double Buffer에 직접 쓰기) */
    if (_CM_UnpackPDO(data, len) > 0) {
        /* SDO-managed fields carry-over: swap 시 SDO 값 유실 방지 */
        uint8_t read_idx = 1 - write_idx;
        CM_RxData_t* read_buf = &s_cm_double_buf.buffers[read_idx];
        write_buf->suitMode        = read_buf->suitMode;
        write_buf->suitAssistLevel = read_buf->suitAssistLevel;
        write_buf->isPVectorRHDone = read_buf->isPVectorRHDone;
        write_buf->isPVectorLHDone = read_buf->isPVectorLHDone;

        _CM_DecodeRxPDOData(write_buf);
        s_cm.isDataReady = true;

        /* ✅ Lock-Free Swap (volatile uint8_t는 ARM에서 Atomic) */
        s_cm_double_buf.write_idx = 1 - write_idx;  // 0↔1 toggle (Atomic)
    }
}

// --- 데이터 수신 API (Data Getter API) ---
/**
 * @brief CM PDO 데이터 가져오기 (Lock-Free Double Buffer)
...
bool CM_GetRxData(CM_RxData_t* rxData)
{
    ...
    /* ✅ Lock-Free: ISR이 쓰지 않는 반대 버퍼 읽기 */
    uint8_t read_idx = 1 - s_cm_double_buf.write_idx;  /* 반대 버퍼 선택 */
    memcpy(rxData, &s_cm_double_buf.buffers[read_idx], sizeof(CM_RxData_t));
...
{
    /* ✅ Task Context에서 호출: Lock 불필요 */
    /* Double Buffer는 ISR에서만 쓰므로, 여기서는 rawPdoData만 리셋 */
    memset(&s_cm.rawPdoData, 0, sizeof(s_cm.rawPdoData));
    memset(&s_cm_double_buf, 0, sizeof(s_cm_double_buf));  /* Double Buffer도 리셋 */
 ...
 * @note Task Context에서 호출: 읽기 버퍼에 업데이트
 */
void CM_UpdateSuitMode(CM_SuitMode_t newMode)
{
    uint8_t read_idx = 1 - s_cm_double_buf.write_idx;
    s_cm_double_buf.buffers[read_idx].suitMode = newMode;
}
...
{
    uint8_t read_idx = 1 - s_cm_double_buf.write_idx;
    s_cm_double_buf.buffers[read_idx].suitAssistLevel = newLevel;
}
...
{
    uint8_t read_idx = 1 - s_cm_double_buf.write_idx;
    s_cm_double_buf.buffers[read_idx].isPVectorRHDone = isCompleted;
}

void CM_UpdatePVectorCompletedLH(uint8_t isCompleted)
{
    uint8_t read_idx = 1 - s_cm_double_buf.write_idx;
    s_cm_double_buf.buffers[read_idx].isPVectorLHDone = isCompleted;
}
...
{
    /* ✅ Task Context에서 호출: Double Buffer의 현재 읽기 버퍼에 접근 */
    uint8_t read_idx = 1 - s_cm_double_buf.write_idx;
    CM_RxData_t* read_buf = &s_cm_double_buf.buffers[read_idx];
    
    if (nodeId == SYS_NODE_ID_RH) {
        read_buf->isPVectorRHDone = false;
    } else if (nodeId == SYS_NODE_ID_LH) {
        read_buf->isPVectorLHDone = false;
    }
}
...
{
    // --- Loop & State Data ---
    dst_buf->suitAssistModeLoopCnt = s_cm.rawPdoData.suitAssistModeLoopCnt;
