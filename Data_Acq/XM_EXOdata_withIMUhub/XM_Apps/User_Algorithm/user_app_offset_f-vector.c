...
static float FVecDecoder_StepSingle(FVecSlot_t buf[], float *p_offset);
...
static void Active_Loop(void)
{
    ...
    if (currEnable == Enable_ON) {
        ...
        for (int n = 0; n < 2; ++n) {
            LH_Torque = FVecDecoder_StepSingle(s_fvec_buf_LH, &s_offset_LH);
            RH_Torque = FVecDecoder_StepSingle(s_fvec_buf_RH, &s_offset_RH);
        }

        if (FVecDecoder_IsBufIdle(s_fvec_buf_LH)) s_offset_LH = 0.0f;
		if (FVecDecoder_IsBufIdle(s_fvec_buf_RH)) s_offset_RH = 0.0f;
...
static float FVecDecoder_StepSingle(FVecSlot_t buf[], float *p_offset)
{
    ...
    for (int i = 0; i < F_VECTOR_BUFF_SIZE; i++) {
        ...
        // Step 1. delay 처리 (기존 동일)
        if (s->time_stamp == s->delay) {
            s->u = s->tau_max;
        }

        // Step 2. IIR 필터 계산 (기존 동일)
