...
static void _ParseMTData2(XsensParser_t* parser, XsensMTi_packet_t* output)
{
    ...
    uint16_t idx = 0;
    /* [R4] TLV 경계가드: 헤더(Data ID 2B + Len 1B) 3바이트가 확보될 때만 진입 */
    while ((uint32_t)idx + 3U <= parser->len) {
        // 1. Data ID 읽기 (2 bytes, Big-endian)
        uint16_t data_id = ((uint16_t)parser->buffer[idx] << 8) | parser->buffer[idx+1];
        // 2. Data Length 읽기 (1 byte)
        uint8_t data_len = parser->buffer[idx+2];
        idx += 3;

        /* [R4] 페이로드가 버퍼를 넘기면(절단/손상 프레임) 중단 — OOB read 방지 */
        if ((uint32_t)idx + data_len > parser->len) {
            break;
        }

        // 3. Data ID에 따라 파싱 (필드별 최소 길이 확인 — data_len 부족 시 skip)
        switch(data_id) {
...
static volatile XsensMTi_packet_t  s_imu_datalake[XSENS_MAX_INSTANCES];
#if !defined(USE_FREERTOS)
#define XSENS_SEQLOCK_MAX_RETRY  4U   /**< writer 가 매우 짧아 0~1 회면 충분, cap=종료보장 */
static volatile uint32_t s_imu_seq[XSENS_MAX_INSTANCES];  /**< seqlock: 홀수=쓰기중 */
#endif
...
void XsensMTi_StateInit(uint8_t ch)
{
    ...
    memset((void*)&s_imu_datalake[ch], 0, sizeof(s_imu_datalake[ch]));
#if !defined(USE_FREERTOS)
    s_imu_seq[ch] = 0;
#endif
...
void XsensMTi_UpdateData(uint8_t ch, const XsensMTi_packet_t* packet)
{
    ...
    s_imu_seq[ch]++;             /* 홀수 = 쓰기 시작 */
    __DMB();
    s_imu_datalake[ch] = *packet;
    __DMB();
    s_imu_seq[ch]++;             /* 짝수 = 쓰기 완료 */
#endif
     ...
     * cap 소진(사실상 불가) 시 false → 호출부(FetchAllImus)가 해당 ch 0 처리. */
    {
        ...
        do {
            ...
            *out = s_imu_datalake[ch];
            __DMB();
            s2 = s_imu_seq[ch];
        } while ((s1 != s2 || (s1 & 1U)) && (++retry < XSENS_SEQLOCK_MAX_RETRY));
        if (s1 != s2 || (s1 & 1U)) {
