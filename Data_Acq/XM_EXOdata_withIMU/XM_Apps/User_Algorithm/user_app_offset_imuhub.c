...
static void UpdateStatusFields(void);
...
//    }
	...
	GaussDecoder_Init();   // Gaussian 토크 프로파일 버퍼 초기화
...
//    }
    ...
    // USB CDC 스트리밍 데이터 소스 등록 ("AGRB MON START" 명령용)
    SavingData.sof = Transmit_SOF;
    SavingData.len = (uint16_t)sizeof(SavingData_t);
    XM_SetUsbStreamSource(&SavingData, sizeof(SavingData_t));
}
...
static void Standby_Loop(void)
{
    ...
    // Button 이벤트를 먼저 처리 (H10 모드 전환 전에 SmartAssist 반영)
    ...
    // H10 모드 체크 (Button 처리 후 전환하여 SmartAssist 값이 Active_Entry에 반영됨)
    if (XM.status.h10.h10Mode == XM_H10_MODE_ASSIST) {
        XM_TSM_TransitionTo(s_userHandle, XM_STATE_ACTIVE);
    }

    // 상태 필드는 항상 갱신 (PC에서 모드 확인용)
    UpdateStatusFields();

    // Recording ON일 때만 센서 데이터 + loopCnt 갱신
...
static void UpdateStatusFields(void)
{
    // 상태 필드는 Recording과 무관하게 항상 갱신 (PC에서 모드 확인용)
...
static void FillAndSendSavingData(void)
{
    ...
    // CRC 계산 (프레임워크가 전송 시 사용)
    SavingData.crc = CalcCrc16((const uint8_t*)&SavingData,
                               sizeof(SavingData_t) - sizeof(SavingData.crc));

    // XM_SetUsbStreamSource로 등록했으므로 별도 전송 불필요
    // 프레임워크가 USB 요청 시 자동으로 SavingData 버퍼를 전송함
}
...
static float GaussDecoder_StepSingle(GaussSlot_t buf[], float *p_offset)
{
    ...
    for (int i = 0; i < GAUSS_SLOT_MAX; i++) {
        ...
        float raw   = expf(-0.5f * powf(diff, s->n));
...
static void ProcessFVecPacket(const uint8_t* payload, uint16_t payload_len)
{
    ...
    //      되도록 한다.
        ...
        // Packet format: [traj_id][tau_max,mu,sigma,t_end]...[-1,0,0,0] repeat per traj block
        uint16_t j = 0u;
