...
// 2ms마다 300바이트가 쏟아져도 100ms를 버티는 크기 (300B*50 = 15KB)
// 16KB로 설정
//#define CDC_TX_RING_BUFFER_SIZE  (16 * 1024)
//#define CDC_RX_RING_BUFFER_SIZE  (16 * 1024)

#define CDC_TX_RING_BUFFER_SIZE  CDC_TX_BUFFER_SIZE
#define CDC_RX_RING_BUFFER_SIZE  CDC_RX_BUFFER_SIZE
...
{
    if (len == 0 || len > CDC_TX_BUFFER_SIZE) return false;

    uint32_t head = atomic_load(&s_txHead);
    uint32_t tail = s_txTail; // ISR 변수지만 volatile 읽기 안전

    // 공간 계산
    uint32_t free = (head >= tail) ? (CDC_TX_BUFFER_SIZE - (head - tail) - 1) : ((tail - head) - 1);
    if (len > free) return false; // 버퍼 꽉 참

    // 데이터 복사 (Wrap-around)
    const uint8_t* pData = (const uint8_t*)data;
    uint32_t chunk1 = (head + len > CDC_TX_BUFFER_SIZE) ? (CDC_TX_BUFFER_SIZE - head) : len;
    ...
    atomic_store(&s_txHead, (head + len) % CDC_TX_BUFFER_SIZE);
...
uint32_t CdcStream_Read(void* buf, uint32_t max_len)
{
    ...
    uint32_t available = (head >= tail) ? (head - tail) : (CDC_RX_BUFFER_SIZE - tail + head);
    uint32_t read_len = (available > max_len) ? max_len : available;

    uint8_t* pBuf = (uint8_t*)buf;
    uint32_t chunk1 = (tail + read_len > CDC_RX_BUFFER_SIZE) ? (CDC_RX_BUFFER_SIZE - tail) : read_len;
    
    memcpy(pBuf, &s_cdcRxRingBuffer[tail], chunk1);
    if (read_len > chunk1) memcpy(pBuf + chunk1, &s_cdcRxRingBuffer[0], read_len - chunk1);

    atomic_store(&s_rxTail, (tail + read_len) % CDC_RX_BUFFER_SIZE);
    return read_len;
}
...
void CdcStream_OnRxReceived(uint8_t* data, uint32_t len)
{
    ...
    uint32_t free = (tail > head) ? (tail - head - 1) : (CDC_RX_BUFFER_SIZE - (head - tail) - 1);
    if (len > free) return; // 오버플로우 (데이터 버림)

    // 링버퍼 복사
    uint32_t chunk1 = (head + len > CDC_RX_BUFFER_SIZE) ? (CDC_RX_BUFFER_SIZE - head) : len;
    memcpy(&s_cdcRxRingBuffer[head], data, chunk1);
    if (len > chunk1) memcpy(&s_cdcRxRingBuffer[0], data + chunk1, len - chunk1);

    s_rxHead = (head + len) % CDC_RX_BUFFER_SIZE; // ISR에서만 쓰므로 atomic 불필요
}
...
static void _TryTransmit(void)
{
    ...
    // 2. 연속된 데이터 길이 계산
    uint32_t len = (head > tail) ? (head - tail) : (CDC_TX_BUFFER_SIZE - tail);
    ...
    if (CDC_Transmit_FS(&s_cdcTxRingBuffer[tail], (uint16_t)len) == USBD_OK) {
        s_txTail = (tail + len) % CDC_TX_BUFFER_SIZE;
