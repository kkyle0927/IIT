 ...
 * @details [변경] CM_XM_Link_IsConnected() → CM_Drv_IsConnected() (.cursorrules Phase 5)
 */
bool XM_IsCmConnected(void)
{
    return CM_Drv_IsConnected();
}

/**
 * @brief 현재 CM과의 PnP(NMT) 상태를 가져옵니다.
 * @details [변경] CM_XM_Link_GetXMNmtState() → CM_Drv_GetNmtState() (.cursorrules Phase 5)
 */
CM_NmtState_t XM_GetXMNmtState(void)
{
    return CM_Drv_GetNmtState();
}
...
static uint32_t s_loop_count_base = 0;

void XM_CaptureLoopCountBase(void)
{
    s_loop_count_base = XM.status.h10.h10AssistModeLoopCnt;
}

uint32_t XM_GetRelativeLoopCount(void)
{
    return XM.status.h10.h10AssistModeLoopCnt - s_loop_count_base;
}
