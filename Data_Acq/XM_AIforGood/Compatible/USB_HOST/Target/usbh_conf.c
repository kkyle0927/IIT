...
__weak void HAL_HCD_MspInit(HCD_HandleTypeDef* hcdHandle)
{
  ...
  */
    ...
    */
     ...
     * [수정] Priority 5→6: FDCAN(5)보다 낮은 우선순위로 설정하여
     * USB Bulk Transfer 완료 ISR이 FDCAN 수신 ISR을 지연시키지 않도록 함.
     * Priority 6은 configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY(5) 이상이므로
     * FreeRTOS ISR-safe API 호출에 안전함. */
...
{
  /* Re-activate the host channel by calling HC_Init with current parameters */
  HCD_HandleTypeDef *pHandle = phost->pData;
  HAL_StatusTypeDef hal_status;

  hal_status = HAL_HCD_HC_Init(pHandle, pipe,
                               pHandle->hc[pipe].ep_num,
                               pHandle->hc[pipe].dev_addr,
                               pHandle->hc[pipe].speed,
                               pHandle->hc[pipe].ep_type,
                               pHandle->hc[pipe].max_packet);
