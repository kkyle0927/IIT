...
#define DOP_COMMON_SDO_CB(t_task)                                                                               \
    ...
    {                                                                                                           \
        ...
        res->status = DOP_SDO_SUCC;                                                                             \
                                                                                                                ...
                                                                                                                \
    static void GetRoutineIDCB(DOP_SDOArgs_t* req, DOP_SDOArgs_t* res)                                          \
    {                                                                                                           \
        res->status = DOP_SDO_SUCC;                                                                             \
        ...
        for (int i = 0; i < res->dataSize; ++i) {                                                               \
            ((uint8_t*)res->data)[i] = (uint8_t)((t_task).routine.id[i]);                                       \
                                                                                                                ...
                                                                                                                \
    static void SetRoutineCB(DOP_SDOArgs_t* req, DOP_SDOArgs_t* res)                                            \
    {                                                                                                           \
        res->dataSize = 0;                                                                                      \
        res->status = DOP_SDO_SUCC;                                                                             \
                                                                                                                \
        RoutineObj_t* active_routine_obj = &(t_task).routine;                                                   \
        uint8_t* new_id_list = (uint8_t*)req->data;                                                             \
        int new_id_count = req->dataSize;                                                                       \
                                                                                                                \
        for (int i = active_routine_obj->numOfRoutineID - 1; i >= 0; i--)                                       \
        {                                                                                                       \
            uint8_t current_id = active_routine_obj->id[i];                                                     \
            bool found_in_new_list = false;                                                                     \
            for (int j = 0; j < new_id_count; j++) {                                                            \
                if (current_id == new_id_list[j]) {                                                             \
                    found_in_new_list = true;                                                                   \
                    break;                                                                                      \
                }                                                                                               \
            }                                                                                                   \
            if (!found_in_new_list) {                                                                           \
                PopRoutine(active_routine_obj, current_id);                                                     \
            }                                                                                                   \
        }                                                                                                       \
                                                                                                                \
        for (int i = 0; i < new_id_count; i++)                                                                  \
        {                                                                                                       \
            uint8_t new_id = new_id_list[i];                                                                    \
            PushRoutine(active_routine_obj, new_id);                                                            \
        }                                                                                                       \
                                                                                                                \
        res->status = DOP_SDO_SUCC;                                                                             \
    }                                                                                                           \
                                                                                                                \
    static void ClearAllRoutinesCB(DOP_SDOArgs_t* req, DOP_SDOArgs_t* res)                                      \
    {                                                                                                           \
...
/**
 * @brief Create all common SDOs
 * 
 * @ adc The ADC for the conversion.
...
#define DOP_COMMON_SDO_CREATE(t_taskID)                                                                         \
    ...
    DOP_CreateSDO(t_taskID, 2, DOP_UINT8, GetRoutineIDCB);                                                      \
    DOP_CreateSDO(t_taskID, 3, DOP_UINT8, SetRoutineCB);                                                        \
    DOP_CreateSDO(t_taskID, 4, DOP_UINT8, ClearAllRoutinesCB);                                                  \
