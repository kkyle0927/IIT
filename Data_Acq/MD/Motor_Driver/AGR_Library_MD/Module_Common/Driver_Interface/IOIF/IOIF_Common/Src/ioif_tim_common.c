...
void IOIF_SetTimRCR(IOIF_Tim_t timer, uint32_t RCRVal)
{
    /* If RCR is set before Timer CNT starts operation, UEV operates on underflow.
                     After Timer CNT starts operating, setting RCR immediately will cause UEV to function on Overflow.*/
    if (timer < 0 || timer >= IOIF_TIM_COUNT) {
        return;
    }

    BSP_SetTimRCR((BSP_Tim_t)timer, RCRVal);
