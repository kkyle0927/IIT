...
echo "[INFO] Running compare_results.py on $(hostname) in CPU-only mode"
echo "[INFO] Arguments: $@"
...
if [ -z "$ram_per_worker_gb" ]; then
    if [ "$workers" -gt 1 ]; then
        ram_per_worker_gb=$(( 5 / workers ))
        if [ "$ram_per_worker_gb" -lt 2 ]; then
            ram_per_worker_gb=2
        fi
    else
        ram_per_worker_gb=5
...
if [ "$workers" -eq 1 ]; then
    ...
    echo "[INFO] Single-worker CPU eval. threads=${threads_per_worker}, ram_limit_gb=${ram_per_worker_gb}, skip_fi=${skip_feature_importance}"
...
echo "[INFO] Parallel CPU eval. workers=${workers}, threads_per_worker=${threads_per_worker}, ram_limit_gb=${ram_per_worker_gb}, skip_fi=${skip_feature_importance}"
