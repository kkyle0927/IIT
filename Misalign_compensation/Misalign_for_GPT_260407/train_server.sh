#!/bin/bash
# ==============================================================
# SLURM JOB SCRIPT — Misalign_compensation
# --------------------------------------------------------------
# [Server Information]
#   NodeName    : server1
#   Total CPUs  : 64 cores
...
if [ -z "$SLURM_JOB_ID" ]; then
    ...
    confirm_and_submit() {
        ...
        echo "  GPU 내 worker : 기본 2개, 적응형 1~3개"
        ...
        if command -v whiptail &>/dev/null; then
            whiptail --title "제출 확인" \
                --yesno "${num_launchers}개 launcher job을 제출하시겠습니까?\n\n동시 GPU: ${MAX_PARALLEL} | GPU 내 worker: 기본 2개, 적응형 1~3개" \
        ...
        for ((slot=0; slot<num_launchers; slot++)); do
            ...
            job_id=$(sbatch --parsable \
                ...
                --cpus-per-task="8" \
                "$0" "${launcher_cfgs[@]}")
            echo "  Submitted: $job_name (JobID: $job_id, configs=${#launcher_cfgs[@]}) on $gpu_assign"
        done
...
if [ ${#CONFIG_PATHS[@]} -eq 0 ]; then
    echo "ERROR: config 경로가 지정되지 않았습니다. 사용법: sbatch train_server.sh <config1.yaml> [config2.yaml ...]"
    exit 1
fi
...
TRAIN_LOW_UTIL_PCT="${TRAIN_LOW_UTIL_PCT:-55}"
TRAIN_HIGH_UTIL_PCT="${TRAIN_HIGH_UTIL_PCT:-92}"
TRAIN_LOW_MEM_PCT="${TRAIN_LOW_MEM_PCT:-35}"
TRAIN_HIGH_MEM_PCT="${TRAIN_HIGH_MEM_PCT:-78}"
TRAIN_MIN_THREADS_PER_WORKER="${TRAIN_MIN_THREADS_PER_WORKER:-2}"
TRAIN_RECOVERY_SUCCESS_STREAK="${TRAIN_RECOVERY_SUCCESS_STREAK:-1}"
...
current_target="$TRAIN_DEFAULT_WORKERS"
if [ "$current_target" -lt "$TRAIN_MIN_WORKERS" ]; then
    current_target="$TRAIN_MIN_WORKERS"
fi
if [ "$current_target" -gt "$TRAIN_MAX_WORKERS" ]; then
    current_target="$TRAIN_MAX_WORKERS"
fi

echo "[SCHED] Adaptive same-GPU workers enabled: default=${TRAIN_DEFAULT_WORKERS}, current=${current_target}, range=${TRAIN_MIN_WORKERS}-${TRAIN_MAX_WORKERS}, retries=${TRAIN_MAX_RETRIES}"
if [ "$TRAIN_ADAPTIVE_WORKERS" != "1" ]; then
    echo "[SCHED] GPU util-based worker scaling disabled. OOM backoff/recovery policy is active."
fi
...
gpu_stats() {
    local line
    line=$(nvidia-smi --query-gpu=utilization.gpu,memory.used,memory.total --format=csv,noheader,nounits 2>/dev/null | head -n 1 | tr -d ' ')
    if [ -z "$line" ]; then
        return 1
    fi
    IFS=',' read -r util mem_used mem_total <<<"$line"
...
adjust_target_workers() {
    ...
    if ! read -r util mem_used mem_total < <(gpu_stats); then
        return
    fi
    mem_pct=$(( mem_used * 100 / mem_total ))

    echo "[SCHED] GPU util=${util}% mem=${mem_used}/${mem_total}MiB (${mem_pct}%), target=${current_target}, active=${#active_pids[@]}, queued=${#cfg_queue[@]}"

    if [ "$mem_pct" -ge "$TRAIN_HIGH_MEM_PCT" ] || [ "$util" -ge "$TRAIN_HIGH_UTIL_PCT" ]; then
        high_pressure_streak=$((high_pressure_streak + 1))
        low_headroom_streak=0
        if [ "$high_pressure_streak" -ge 2 ] && [ "$current_target" -gt "$TRAIN_MIN_WORKERS" ]; then
            current_target=$((current_target - 1))
            high_pressure_streak=0
            echo "[SCHED] High GPU pressure detected. Reducing target workers to ${current_target}."
        fi
    elif [ "$mem_pct" -le "$TRAIN_LOW_MEM_PCT" ] && [ "$util" -le "$TRAIN_LOW_UTIL_PCT" ]; then
        low_headroom_streak=$((low_headroom_streak + 1))
        high_pressure_streak=0
        if [ "$low_headroom_streak" -ge 2 ] && [ "$current_target" -lt "$TRAIN_MAX_WORKERS" ]; then
            if [ $(( ${#active_pids[@]} + ${#cfg_queue[@]} )) -ge $(( current_target + 1 )) ]; then
                current_target=$((current_target + 1))
                low_headroom_streak=0
                echo "[SCHED] GPU headroom detected. Increasing target workers to ${current_target}."
            fi
        fi
    else
        low_headroom_streak=0
...
launch_worker() {
    ...
    (
        ...
        python model_training.py --config "$cfg" >"$worker_log" 2>&1
    ) &
...
handle_finished_workers() {
    ...
    for i in "${!active_pids[@]}"; do
        ...
        if [ "$status" -eq 0 ]; then
            ...
            if [ "$current_target" -lt "$TRAIN_DEFAULT_WORKERS" ]; then
                recovery_success_streak=$((recovery_success_streak + 1))
                if [ "$recovery_success_streak" -ge "$TRAIN_RECOVERY_SUCCESS_STREAK" ]; then
                    current_target="$TRAIN_DEFAULT_WORKERS"
                    if [ "$current_target" -gt "$TRAIN_MAX_WORKERS" ]; then
                        current_target="$TRAIN_MAX_WORKERS"
                    fi
                    ...
                    low_headroom_streak=0
                    high_pressure_streak=0
                    echo "[SCHED] Stable completion detected. Restoring target workers to ${current_target}."
                fi
            else
                recovery_success_streak=0
            fi
...
# Direct:       sbatch train_server.sh configs/a.yaml [configs/b.yaml ...]
