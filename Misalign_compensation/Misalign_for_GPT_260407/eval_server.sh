...
full_ram_gb="${EVAL_FULL_RAM_GB:-15}"
ablation_ram_gb="${EVAL_ABLATION_RAM_GB:-6}"
tmp_dir="/tmp/misalign-eval-${SLURM_JOB_ID:-manual}"
mkdir -p "$tmp_dir"
...
if [ "$workers" -eq 1 ]; then
    ...
    echo "[INFO] Parallel fast eval on one GPU. workers=${workers}, threads_per_worker=${threads_per_worker}, ram_limit_gb=${ram_per_worker_gb}"
