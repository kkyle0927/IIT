...
# Limit to 1 GPU (allow override for same-GPU shard workers)
_VISIBLE_GPU = os.environ.get("COMPARE_RESULTS_CUDA_VISIBLE_DEVICES", "0")
...
# Limit CPU Threads to configured worker allowance
...
def get_args():
    ...
    parser.add_argument('--metrics-only', action='store_true', help='Only compute and save metrics summary without generating plots')
    parser.add_argument('--summary-output', type=str, default=None, help='Optional CSV path for automated metrics summary output')
    parser.add_argument('--metrics-summary-csv', type=str, default=None, help='Reuse precomputed metrics summary CSV for ablation report')
...
def get_compare_result_dir(exp_path):
    exp_path = Path(exp_path)
    fold_name = exp_path.name
    try:
        rel_parent = exp_path.relative_to(Path("experiments")).parent
    except ValueError:
        rel_parent = Path(exp_path.parent.name)
    if fold_name.startswith("fold_"):
        exp_id = exp_path.parent.name
        return Path("compare_result") / rel_parent / exp_id / fold_name
    return Path("compare_result") / rel_parent / fold_name


def discover_experiment_dirs(target_dir, filter_pattern=None):
    if not target_dir:
    ...
    if filter_pattern:
        ...
        for p in patterns:
            exp_dirs.extend(
                candidate
                for candidate in root_path.rglob(p.strip())
                if candidate.is_dir() and (candidate / "model.pt").exists()
            )
        return sorted(set(exp_dirs), key=lambda x: str(x))
...
def get_auto_summary_dir(valid_dirs):
    if not valid_dirs:
        return Path("compare_result")
    try:
        rel_paths = [p.relative_to(Path("experiments")) for p in valid_dirs]
        common_parent = Path(os.path.commonpath([str(p.parent) for p in rel_paths]))
        return Path("compare_result") / common_parent
    except Exception:
        return Path("compare_result")


def save_automated_metrics_summary(results, output_csv):
    output_csv = Path(output_csv)
    output_csv.parent.mkdir(exist_ok=True, parents=True)
    if not results:
        print(f"[AUTO] No results to save for {output_csv}")
        return output_csv
    df = pd.DataFrame(results).sort_values(["mae", "rmse", "name"], ascending=[True, True, True])
    df.to_csv(output_csv, index=False)
    print(f"[AUTO] Saved metrics summary to {output_csv}")
    return output_csv
...
from model_training import (
    TCN_MLP, StanceGatedTCN, LoadGatedTCN, GroupedAdditiveLinear, FactorizedBranchTCN, AttentionTCN, TCN_GRU, LazyWindowDataset, build_nn_dataset, build_nn_dataset_multi,
...
def build_residual_reference_signal(robot_ref_list, X_list, window_size, window_output, stride_eval, est_tick_ranges, template):
    """
    Build the robot reference signal aligned with evaluation outputs without
    materializing a full LazyWindowDataset over all windows.
    """
    if est_tick_ranges is None and window_output > 1:
        return None

    sampled_trials = []
    for trial_x, robot_ref in zip(X_list, robot_ref_list):
        length = len(trial_x)
        max_lookahead = window_output if est_tick_ranges is None else max(est_tick_ranges)
        limit = length - window_size - max_lookahead + 1
        if limit <= 0:
        ...
        starts = list(range(0, limit, stride_eval))
        if not starts:
            continue

        if est_tick_ranges is not None:
            tick_offsets = np.asarray(est_tick_ranges, dtype=np.int64)
            frames = [
                robot_ref[start + window_size + tick_offsets - 1]
                for start in starts
            ]
            trial_arr = np.stack(frames, axis=0)
        else:
            frames = [
                robot_ref[start + window_size : start + window_size + window_output]
                for start in starts
            ]
            trial_arr = np.stack(frames, axis=0)

        sampled_trials.append(trial_arr)

    if not sampled_trials:
        return None

    robot_ref = np.concatenate(sampled_trials, axis=0)

    # Match template rank to avoid broadcasting surprises.
    if template is not None:
        if template.ndim == 3 and robot_ref.ndim == 2:
            robot_ref = robot_ref[:, None, :]
        elif template.ndim == 2 and robot_ref.ndim == 3 and robot_ref.shape[1] == 1:
...
def load_and_evaluate(exp_dir, device, return_seqs=False, return_extras=False):
    ...
    if model_type == "StanceGatedTCN":
        ...
        branch_indices = {name: resolve_feature_indices(feat_names_simple, patterns) for name, patterns in branch_patterns.items()}
        ...
        gate_indices = resolve_feature_indices(feat_names_simple, gate_patterns)
        model = LoadGatedTCN(
            **common_args,
    ...
    if use_residual and Y_abs_list:
        ...
        robot_ref = build_residual_reference_signal(
            robot_ref_list, X_list,
            window_size, window_output, stride_eval, est_tick_ranges,
            y_pred,
        )
        if robot_ref is None:
            # Fallback for sequence-output cases not covered by the lightweight path.
...
def analyze_single_folder(folder_path):
    ...
    if not os.path.exists(data_path): 
        if os.path.exists("combined_data_S008.h5"): data_path = "combined_data_S008.h5"
    ...
    save_dir = get_compare_result_dir(folder_path)
    save_dir.mkdir(exist_ok=True, parents=True)
    print(f"  -> Saving plots to: {save_dir}")
    
    # [NEW] Check Overfitting
    plot_training_curves(Path(folder_path), save_dir)
...
def plot_trajectory(y_true, y_pred, title, save_path, series_offsets=None, robot_ref=None):
    """
    ...
    if robot_ref is not None:
        robot_ref = np.squeeze(robot_ref)
        if robot_ref.ndim > 1:
            robot_ref = robot_ref[:, 0]
        robot_ref_plot, _ = insert_breaks(robot_ref, series_offsets)
    else:
        robot_ref_plot = None
    ...
    if robot_ref_plot is not None:
        axes[0].plot(t_plot, robot_ref_plot, label='Calibrated Robot Angle', alpha=0.85, color='#1f77b4', linestyle='-.')
    ...
    if np.sum(mask) > 0:
        ...
        if robot_ref_plot is not None:
            axes[1].plot(t_plot[mask], robot_ref_plot[mask], label='Calibrated Robot Angle', color='#1f77b4', linestyle='-.', alpha=0.85)
    ...
    with h5py.File(data_path, 'r') as f:
        for cond in actual_conds:
            ...
            for col_idx, lv_name in enumerate(target_lvs):
                ...
                # Plot Configuration:
                # Use 3 rows total: [Full, Zoom1, Zoom2].
...
def analyze_automated(
    ...
    exp_dirs = discover_experiment_dirs(target_dir, filter_pattern)
    ...
    if summary_output:
        summary_csv = Path(summary_output)
    else:
        summary_dir = get_auto_summary_dir(valid_dirs)
        suffix = f"_shard{shard_index + 1}of{num_shards}" if num_shards > 1 else ""
        summary_csv = summary_dir / f"automated_eval_metrics_summary{suffix}.csv"
    ...
    for idx, exp_path in enumerate(valid_dirs, 1):
        _t0 = _time.time()
        fold_name = exp_path.name
        fold_output_dir = get_compare_result_dir(exp_path)
        ...
        try:
            if metrics_only:
                res = load_and_evaluate(exp_path, device, return_seqs=False, return_extras=False)
            else:
            ...
            if metrics_only:
                print(
                    f"  [FAST] {fold_name}: MAE={res['mae']:.4f}, "
                    f"RMSE={res['rmse']:.4f}, R²={res.get('r2', 0.0):.4f}, Lag={res.get('lag_ms', 0.0):.1f}ms"
                )
            ...
            if "y_true_seq" in res and "y_pred_seq" in res:
                ...
                plot_trajectory(
                    ...
                    f"Trajectory: {fold_name}",
                    traj_path,
                    series_offsets=res.get("series_offsets"),
                    robot_ref=res.get("robot_ref_seq"),
                )
    ...
    save_automated_metrics_summary(results, summary_csv)
...
def analyze_detailed_single(exp_path):
    ...
    plot_dir = get_compare_result_dir(exp_path)
    plot_dir.mkdir(parents=True, exist_ok=True)

    # [NEW] Plot Training Curves (Overfitting Check)
    exp_dir_path = exp_path
...
def analyze_ablation_report(metrics_summary_csv=None):
    print("Starting Ablation Analysis Report...")
    # device is global in this script
    
    # Check if compare_result exists
    output_dir = Path(metrics_summary_csv).parent if metrics_summary_csv else Path("compare_result")
    output_dir.mkdir(exist_ok=True)

    if metrics_summary_csv:
        metrics_summary_csv = Path(metrics_summary_csv)
        if not metrics_summary_csv.exists():
            print(f"[ERROR] Metrics summary CSV not found: {metrics_summary_csv}")
            return
        print(f"Using precomputed metrics summary: {metrics_summary_csv}")
        df = pd.read_csv(metrics_summary_csv)
    else:
...
if __name__ == "__main__":
    args = get_args()
    if args.ablation:
        analyze_ablation_report(metrics_summary_csv=args.metrics_summary_csv)
    elif args.auto:
        analyze_automated(
            args.target_dir,
            args.filter,
        ...
        if not args.skip_ablation:
            analyze_ablation_report(metrics_summary_csv=args.summary_output if args.metrics_only else None)
    else:
        main()
