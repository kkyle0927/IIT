...
- 새 archive를 설계할 때는 accuracy, latency, parameter count, sensor reduction, deployability 중 어떤 기여 축을 노리는지 먼저 명시할 것. 그 뒤 해당 축에서 baseline 대비 기대 이점을 수치 또는 구조 차이로 설명할 것
- 사용자가 병렬 조사, delegation, subagent 활용을 명시적으로 요청한 경우에는 subagent를 사용할 것. 사용자가 명시적으로 요청하지 않은 경우에는 현재 agent가 직접 수행할 것
- 추상적인 슬로건이나 감각적인 표현으로 제안을 정당화하지 말 것. 모델 구조, 입력 feature, 계산량, 실시간성, 비교 기준을 근거로 설명할 것
...
- `experiments/archive_seasonX/archive_Y/...`를 평가하거나 비교할 때 결과 산출물은 반드시 `compare_result/archive_seasonX/archive_Y/...` 아래에 저장할 것
- `configs/archive_seasonX/archive_Y/...`에 대한 학습/평가를 수행할 때 대응 평가 결과 저장 경로는 반드시 `compare_result/archive_seasonX/archive_Y/`를 기준으로 사용할 것
...
- 평가(GPU): `sbatch -p idxN --job-name="이름" --gres=gpu:idxN:1 eval_server.sh --auto`
- 평가(CPU): `sbatch -p idxN --job-name="이름" cpu_eval.sh --auto`
- 로컬 테스트: `conda run -n IIT python model_training.py --config configs/...`
- `train_server.sh`에 config 여러 개를 넘기면 같은 GPU 안에서 worker pool로 학습한다. 기본 worker 수는 3개이며, 기본 설정에서는 GPU util 기반 자동 조절을 사용하지 말고 OOM 발생 시 다음 launch 수를 3→2→1로 낮춘 뒤 성공 완료가 발생하면 다시 3으로 복구할 것
- `cpu_eval.sh`는 GPU를 요청하지 말 것. `--cpus-per-task=4`, `--mem=5G`, `CUDA_VISIBLE_DEVICES=""`, `OMP_NUM_THREADS=4`, `MKL_NUM_THREADS=4`, `NUMEXPR_NUM_THREADS=4`를 유지할 것
- `cpu_eval.sh`의 내부 RAM 힌트는 총 5GB 기준으로 맞출 것. single worker는 `COMPARE_RESULTS_RAM_GB=5`, multi-worker는 worker당 `2`, 최종 ablation summary는 `4`를 사용할 것
- `eval_server.sh`와 `cpu_eval.sh`는 기본적으로 2단계 평가를 수행할 것. 1차에서는 전체 실험에 대해 `metrics-only` 평가를 수행하고, 2차에서는 MAE 기준 최고 성능 `1개` 실험에 대해서만 full plot을 생성할 것. 기본값은 `EVAL_FULL_TOPK=1`로 둘 것
- 최종 ablation summary는 1차 fast eval에서 저장한 metrics summary CSV를 재사용할 것. ablation summary 때문에 전체 실험을 다시 평가하지 말 것
- Slurm 로그로 리다이렉트되는 non-interactive 학습에서는 batch/validation `tqdm`를 출력하지 말 것. 학습 로그에는 epoch 요약과 주요 이벤트만 남길 것
- 본 프로젝트에서 학습하는 모델은 추후 실시간 inference에 사용할 것을 전제로 설계할 것. 전처리, 파생 feature 생성, 모델 추론을 합친 전체 파이프라인은 online 추론이 가능할 정도로 가벼워야 하며, 실시간 적용이 어려운 무거운 계산이 필요하면 해당 실험을 기본 후보에서 제외할 것
...
- `archive_seasonX/archive_Y`에 대한 train/eval job name은 반드시 `Misalign_ASX_AY_{train|eval}_K` 형식을 사용할 것. 예: `Misalign_AS2_A2_train_1`, `Misalign_AS2_A2_eval_1`
...
├── eval_server.sh         # GPU 기반 SLURM 평가 제출
├── cpu_eval.sh           # CPU-only SLURM 평가 제출
└── combined_data_S008.h5       # symlink → ../Databank/combined_data_S008.h5
```
