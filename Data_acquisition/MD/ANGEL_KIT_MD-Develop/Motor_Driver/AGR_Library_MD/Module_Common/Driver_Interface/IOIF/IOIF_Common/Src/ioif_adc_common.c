...
static uint16_t adc3DmaBuff[IOIF_ADC3_BUFFER_LENGTH] __attribute__((section(".adc3DmaBuff"))) = {0}; // 0 => NTC, 1 => GRF Sensor 1, 2 => GRF Sensor 2
	...
	static uint16_t* adc3Array=NULL;
...
IOIF_ADCState_t IOIF_StartADCDMA(IOIF_ADC_t adc, uint16_t** ppData, uint32_t length)
{
    ...
    switch(adc) {
        ...
        case IOIF_ADC3:
        	*ppData = adc3DmaBuff;
#ifdef _USE_DEBUG_CLI
        	adc3Array=*ppData;// Initialize the global pointer
#endif /* _USE_DEBUG_CLI */
            break;
        default:
...
uint16_t IOIF_GetNTCValue(){
	//	return adc3DmaBuff[0];
	if(adc3Array!=NULL){
		return adc3Array[0]; //NTC value=1rst element
	}
	return 0;//to do error
}
#endif /* _USE_DEBUG_CLI */
