...
#define CLI_DELAY_TIMER_SCALED 100
...
extern IOIF_USB_Tim_t usb_timer_source;
...
void CLI_Delay(uint32_t ms)
{
		...
		osDelay(ms);
#else
//		uint32_t cli_delay_current_time = 0;
//		uint32_t cli_delay_pre_time = BSP_GetCnt(usb_timer_source);
//
//		while(1)
//		{
//			cli_delay_current_time = BSP_GetCnt(usb_timer_source);
//			if(cli_delay_current_time - cli_delay_pre_time > ms * CLI_DELAY_TIMER_SCALED)
//				break;
//		}
...
void cliSystemOff()
{
	...
	CLI_Delay(800);
	HAL_GPIO_WritePin(GPIOG, MCU_SW_CLR_Pin, GPIO_PIN_RESET); // for CM. but for MD?
}
#endif
