...
float VQF_Atan2f(const float y, const float x)
{
    return atan2f(y, x);
}

float VQF_Acosf(const float value)
{
	if (value <= -1.0f) {
		return M_PI;     // π
	}
...
float VQF_Asinf(const float value)
{
    if (value <= -1.0f) {
        return -M_PI_2;  // -π/2
    }
...
void inline quat_local_to_global(float qw, float qx, float qy, float qz,
		float x_in, float y_in, float z_in,
		float* x_out, float* y_out, float* z_out) {
    // 회전 행렬의 직접 계산
    *x_out = x_in*(qw*qw + qx*qx - qy*qy - qz*qz) + y_in*(2*qx*qy - 2*qw*qz) + z_in*(2*qx*qz + 2*qw*qy);
    *y_out = x_in*(2*qx*qy + 2*qw*qz) + y_in*(qw*qw - qx*qx + qy*qy - qz*qz) + z_in*(2*qy*qz - 2*qw*qx);
    *z_out = x_in*(2*qx*qz - 2*qw*qy) + y_in*(2*qy*qz + 2*qw*qx) + z_in*(qw*qw - qx*qx - qy*qy + qz*qz);
}

void inline quat_local_to_global_sagittal(float qw, float qx, float qy, float qz,
                                           float ax_in, float ay_in, float az_in,
    ...
    // Yaw 제거된 quaternion
    float qw_no_yaw = cy * qw - sy * qz;
    float qx_no_yaw = cy * qx - sy * qy;
    float qy_no_yaw = cy * qy + sy * qx;
    float qz_no_yaw = cy * qz + sy * qw;

    // Step 2: 이제 roll, pitch 추출 (yaw 없는 상태에서)
    float roll = atan2f(2.0f * (qw_no_yaw * qx_no_yaw + qy_no_yaw * qz_no_yaw),
                        1.0f - 2.0f * (qx_no_yaw * qx_no_yaw + qy_no_yaw * qy_no_yaw));
    float pitch = asinf(2.0f * (qw_no_yaw * qy_no_yaw - qz_no_yaw * qx_no_yaw));
    ...
    *ax_out = sr * sp * ax_in + cr * ay_in - sr * cp * az_in;
    *ay_out = -cr * sp * ax_in + sr * ay_in + cr * cp * az_in;
    *az_out = cp * ax_in + sp * az_in;
}

void inline GetEulerAnglesWithObject(AS_IMU_Euler_t* euler, \
		AS_IMU_Accel_t* accel_local, AS_IMU_Accel_t* accel_bodyframe, \
		AS_IMU_Gyro_t* gyro_local, AS_IMU_Gyro_t* gyro_global, VQF_t* vqfObject)
{
	float accX = accel_local->x / 9.80665f;
	float accY = accel_local->y / 9.80665f;
	float accZ = accel_local->z / 9.80665f;
	float gyrX = gyro_local->x / 180.0 * M_PI;
	float gyrY = gyro_local->y / 180.0 * M_PI;
	float gyrZ = gyro_local->z / 180.0 * M_PI;

    VQF_Real_t gyr[3] = {gyrX, gyrY, gyrZ};
    VQF_Real_t acc[3] = {accX, accY, accZ};

    VQF_UpdateGyr(vqfObject, (float*)gyr);
    VQF_UpdateAcc(vqfObject, (float*)acc);

    float vqfQuat[4];
    float w, x, y, z;

	VQF_GetQuat9D(vqfObject, vqfQuat);
	...
	float gyroGlobal_x, gyroGlobal_y, gyroGlobal_z;
	float accel_forward, accel_lateral, accel_vertical;

    quat_local_to_global_sagittal(w, x, y, z,
    		accel_local->x, accel_local->y, accel_local->z,
			&accel_forward, &accel_lateral, &accel_vertical);
    quat_local_to_global(w, x, y, z,
    		gyro_local->x, gyro_local->y, gyro_local->z,
			&gyroGlobal_x, &gyroGlobal_y, &gyroGlobal_z);

    accel_bodyframe->x = accel_forward;
    accel_bodyframe->y = accel_lateral;
    accel_bodyframe->z = accel_vertical;
    gyro_global->x = gyroGlobal_x;
    gyro_global->y = gyroGlobal_y;
    gyro_global->z = gyroGlobal_z;

	float sinr_cosp = 2 * (w*x + y*z);
	float cosr_cosp = 1 - 2 * (x*x + y*y);
	euler->roll = VQF_Atan2f(sinr_cosp, cosr_cosp);

	float siny_cosp = 2 * (w*z + x*y);
	float cosy_cosp = 1 - 2 * (y*y + z*z);
	euler->yaw = VQF_Atan2f(siny_cosp, cosy_cosp);

	float sinp = 2 * (w*y - z*x);
	if (fabs(sinp) >= 1)
        euler->pitch = copysign(M_PI / 2, sinp);
	else
        euler->pitch = VQF_Asinf(sinp);

	euler->roll = euler->roll * 57.29577951f;
	euler->pitch = euler->pitch * 57.29577951f;
	euler->yaw = euler->yaw * 57.29577951f;
