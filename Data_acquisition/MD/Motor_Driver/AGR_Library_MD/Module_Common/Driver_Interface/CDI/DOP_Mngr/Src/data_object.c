...
DOP_DataTypeInfo_t DOP_GetDataTypeInfo(DOP_DataType_t t_type)
{
    ...
    case DOP_STRING10:	t_res.typeName = "string10";    t_res.typeSize = 10*sizeof(char);   break;
    default:                                                                                break;
    }
    return t_res;
}
...
{
	if((t_dictID > TASK_NUM) || (t_objID > DOP_SDO_MAX_NUM))	//overflow
		return NULL;			//error
	else
...
{
	if((t_dictID > TASK_NUM) || (t_objID > DOP_PDO_MAX_NUM))		//overflow
		return NULL;			//error
	else
...
//}
void DOP_SetSDOReq(uint8_t t_dictID, uint16_t t_objID, void* t_data, DOP_DataType_t t_type)
{
	DODs[t_dictID].SDOs[t_objID].objID = t_objID;
	DODs[t_dictID].SDOs[t_objID].dataType = t_type;
	DODs[t_dictID].SDOs[t_objID].args.status = DOP_SDO_REQU;
	DODs[t_dictID].SDOs[t_objID].args.data = t_data;
	DODs[t_dictID].SDOs[t_objID].args.dataSize = 1;
	DODs[t_dictID].SDOs[t_objID].args.typeSize = DOP_GetDataTypeInfo(t_type).typeSize;
