...
float RH_IMU_TVCFDeg_sagittal, LH_IMU_TVCFDeg_sagittal;
float RH_IMU_TVCFDeg_frontal, LH_IMU_TVCFDeg_frontal;
float RK_PositionAct;
float LK_PositionAct;
float RK_EncoderVelocity;
float LK_EncoderVelocity;
float RK_IMU_TVCFDeg_sagittal, LK_IMU_TVCFDeg_sagittal;
float RK_IMU_TVCFDeg_frontal, LK_IMU_TVCFDeg_frontal;
...
static void DecodingPDOData(SAM_Motion_Struct* MotionData, int DevIdx)
{
	...
	MotionData->IMU_TVCFDeg_sagittal 	= ScaleInt16ToFloat((GetDevDataSet(DevIdx)->IMU_TVCFDeg_sagittal), DEG_SCALING_FACTOR);
	MotionData->IMU_TVCFDeg_frontal 	= ScaleInt16ToFloat((GetDevDataSet(DevIdx)->IMU_TVCFDeg_frontal), DEG_SCALING_FACTOR);
	MotionData->IMU_TVCFDeg_yaw 		= ScaleInt16ToFloat((GetDevDataSet(DevIdx)->IMU_TVCFDeg_yaw), DEG_SCALING_FACTOR);
...
static void UpdateMotionData(void)
{
	...
	if (RS_File.vec[RH_SAGITAL].usage == 1 && RS_File.vec[LH_SAGITAL].usage == 1 && RS_File.vec[RK_SAGITAL].usage == 0 && RS_File.vec[LK_SAGITAL].usage == 0) {
		...
		RH_IMU_TVCFDeg_sagittal = RH_Sagittal.IMU_TVCFDeg_sagittal;
		LH_IMU_TVCFDeg_sagittal = LH_Sagittal.IMU_TVCFDeg_sagittal;
		RH_IMU_TVCFDeg_frontal = RH_Sagittal.IMU_TVCFDeg_frontal;
		LH_IMU_TVCFDeg_frontal = LH_Sagittal.IMU_TVCFDeg_frontal;
		PelvicSagittalAngle = (RH_IMU_TVCFDeg_sagittal + LH_IMU_TVCFDeg_sagittal)/2 - TrunkNeutralPostureBias;
		PelvicFrontalAngle = (RH_IMU_TVCFDeg_frontal + LH_IMU_TVCFDeg_frontal)/2;
...
//		LH_gyroZCalib = LH_Sagittal.gyroZCalib;
	...
	if (RS_File.vec[RH_SAGITAL].usage == 0 && RS_File.vec[LH_SAGITAL].usage == 0 && RS_File.vec[RK_SAGITAL].usage == 1 && RS_File.vec[LK_SAGITAL].usage == 1) {
		...
		RK_IMU_TVCFDeg_sagittal = RK_Sagittal.IMU_TVCFDeg_sagittal;
		LK_IMU_TVCFDeg_sagittal = LK_Sagittal.IMU_TVCFDeg_sagittal;
		RK_IMU_TVCFDeg_frontal = RK_Sagittal.IMU_TVCFDeg_frontal;
		LK_IMU_TVCFDeg_frontal = LK_Sagittal.IMU_TVCFDeg_frontal;
		PelvicSagittalAngle = (RK_IMU_TVCFDeg_sagittal + LK_IMU_TVCFDeg_sagittal)/2;
		PelvicFrontalAngle = (RK_IMU_TVCFDeg_frontal + LK_IMU_TVCFDeg_frontal)/2;
...
//		LK_gyroZCalib = LK_Sagittal.gyroZCalib;
	...
	if (RS_File.vec[RH_SAGITAL].usage == 1 && RS_File.vec[LH_SAGITAL].usage == 1 && RS_File.vec[RK_SAGITAL].usage == 1 && RS_File.vec[LK_SAGITAL].usage == 1) {
		...
		RH_IMU_TVCFDeg_frontal = RH_Sagittal.IMU_TVCFDeg_frontal;
		LH_IMU_TVCFDeg_frontal = LH_Sagittal.IMU_TVCFDeg_frontal;
		PelvicSagittalAngle = (RH_IMU_TVCFDeg_sagittal + LH_IMU_TVCFDeg_sagittal)/2;
		PelvicFrontalAngle = (RH_IMU_TVCFDeg_frontal + LH_IMU_TVCFDeg_frontal)/2;
...
static void SaveDataMassStorage(void)
{
	...
	if (t_dataCtrlState == TASK_STATE_ENABLE) {
		if (dataSaveMode == DATA_SAVE_ON) { // Data Ctrl Task State Standby -> Enable : DOPC_SetTaskState(TASK_IDX_DATA_CTRL, TASK_STATE_ENABLE)
			...
			sensordata.left_hip.euler.roll = LH_Sagittal.IMU_TVCFDeg_frontal;
			sensordata.left_hip.euler.pitch = LH_Sagittal.IMU_TVCFDeg_sagittal;
			...
			sensordata.right_hip.euler.roll = RH_Sagittal.IMU_TVCFDeg_frontal;
			sensordata.right_hip.euler.pitch = RH_Sagittal.IMU_TVCFDeg_sagittal;
...
static void AppendSDData(SAM_Motion_Struct MotionData, int DevIdx)
{
 ...
 Append_SD_Data_Float16(CONV_FLOAT16, MotionData.IMU_TVCFDeg_sagittal, DEG_ENC_CONSTANT);
 Append_SD_Data_Float16(CONV_FLOAT16, MotionData.IMU_TVCFDeg_frontal, DEG_ENC_CONSTANT);
