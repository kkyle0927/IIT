...
int DOP_Pack_PDO(uint8_t* t_byte_arr, uint8_t* t_byte_len)
{
    ...
    if (pdo_send_list != NULL) {
        for(int i = 0; i < cvector_size(pdo_send_list); ++i) {
            ...
            int temp_cursor = DOP_Convert_PDO_to_Bytes(&pdo_send_list[i], &t_byte_arr[t_cursor]);
            if (temp_cursor > 0) {
...
int DOP_Convert_PDO_to_Bytes(DOP_Header_t* t_header, uint8_t* t_byte_arr)
{
