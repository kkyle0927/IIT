...
static StateEnum_t TransitionMap(StateEnum_t curr, StateEnum_t cmd)
{
    ...
    switch (curr) {
        ...
        case TASK_STATE_OFF:     if (0                      || cmd==TASK_STATE_STANDBY      || 0      	                || cmd==TASK_STATE_ERROR)   {return cmd;} break;
        case TASK_STATE_STANDBY: if (cmd==TASK_STATE_OFF    || 0                            || cmd==TASK_STATE_ENABLE   || cmd==TASK_STATE_ERROR)   {return cmd;} break;
        case TASK_STATE_ENABLE:  if (cmd==TASK_STATE_OFF    || cmd==TASK_STATE_STANDBY      || 0         	            || cmd==TASK_STATE_ERROR)   {return cmd;} break;
        case TASK_STATE_ERROR:   if (cmd==TASK_STATE_OFF    || cmd==TASK_STATE_STANDBY      || cmd==TASK_STATE_ENABLE   || 0                    )   {return cmd;} break;
        default: break;
    }
