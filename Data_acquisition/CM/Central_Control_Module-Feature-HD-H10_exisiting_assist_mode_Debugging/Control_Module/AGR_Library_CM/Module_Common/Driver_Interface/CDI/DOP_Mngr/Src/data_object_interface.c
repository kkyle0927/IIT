...
bool rightHipNPOffsetFlag = false;
bool leftHipNPOffsetFlag = false;
bool rightKneeNPOffsetFlag = false;
bool leftKneeNPOffsetFlag = false;
...
// static int ReadBLESDO(DOPI_DevObj_t* t_obj, uint8_t* t_byteArr);
static int ReadBLESDO(DOPI_DevObj_t* t_obj, uint8_t* t_byteArr, uint32_t buf_len, uint32_t* current_index, uint8_t t_task_id);
...
/* 트랜스미션 버퍼에 첫 번째 SDO ID를 16비트로 나누어 추가하는 함수 */
DOPI_Status_t DOPI_BLEAppendFirstSDOID(DOPI_SDOMsg_t* t_SDOMsg, uint16_t sdoTxID)
{
    /* 메시지 버퍼가 비어 있고 새 데이터를 준비할 수 있는지 확인 */
    if(t_SDOMsg->msgLength != 0) {
        return DOP_STATUS_SDO_SET_FAIL; // 버퍼가 비어 있지 않으므로 SDO ID를 앞부분에 추가할 수 없음
    }

    /* 버퍼의 첫 번째 요소로 sdoTxID의 상위 8비트 추가 */
    t_SDOMsg->txBuf[0] = (uint8_t)(sdoTxID >> 8);  // 상위 8비트 추출 및 저장

    /* 버퍼의 두 번째 요소로 sdoTxID의 하위 8비트 추가 */
    t_SDOMsg->txBuf[1] = (uint8_t)(sdoTxID & 0xFF);  // 하위 8비트 추출 및 저장

    /* SDO ID를 고려하여 메시지 길이 조정 */
    t_SDOMsg->msgLength = 2;

    return DOP_STATUS_SUCCESS;
}

DOPI_Status_t DOPI_BLE_ADD_TASK_ID( DOPI_SDOMsg_t* t_SDOMsg, uint8_t task_id)
{
	uint8_t t_cursor;

	t_SDOMsg->txBuf[0] = task_id;
	t_SDOMsg->msgLength = 2;

	return DOP_STATUS_SUCCESS;
}

DOPI_Status_t DOPI_BLEAppendSDO(DOPI_SDOUnit_t* t_SDOUnit, DOPI_SDOMsg_t* t_SDOMsg)
{
	uint8_t t_cursor;
	...
	if(t_SDOMsg->msgLength == 0) {t_cursor = 2;}
...
int DOPI_UnpackBLESDO(DOPI_DevObj_t* t_obj, uint8_t* t_byteArr, uint32_t buf_len, uint32_t start_index)
{
    uint32_t actual_index = start_index;
    int processed_length = 0; // 처리된 데이터를 저장할 변수

    // Get # of SDOs
    uint8_t t_task_id = t_byteArr[(actual_index)% buf_len];
    uint8_t t_numOfSDO = t_byteArr[(actual_index+1)% buf_len];
    // memcpy(&t_numOfSDO, &t_byteArr[actual_index++], 1);
    actual_index = (actual_index + 2) % buf_len; // 인덱스를 순환 버퍼에 맞게 업데이트

    // Get SDO ID & SDO Data
    if (t_numOfSDO > 0) {
        for (int i = 0; i < t_numOfSDO; ++i) {
            int t_tempCursor = ReadBLESDO(t_obj, t_byteArr, buf_len, &actual_index, t_task_id);
            if (t_tempCursor < 0) {
                // TODO: Unpack SDO ERROR
                return DOP_STATUS_SDO_RECV_FAIL;
            }
...
// int DOPI_UnpackBLESDO(DOPI_DevObj_t* t_obj, uint8_t* t_byteArr)
// {
//     int t_cursor = 0;
...
//     if (t_numOfSDO > 0) {
//         for (int i = 0; i < t_numOfSDO; ++i) {
//             int t_tempCursor = ReadBLESDO(t_obj, &t_byteArr[t_cursor]);
//             if (t_tempCursor > 0) {
//                 t_cursor += t_tempCursor;
//             } else if (t_tempCursor < 0) {
//                 //TODO: Unpack SDO ERROR
...
/* ------------------- RECEIVE NOTI ------------------- */
int DOPI_UnpackNOTI(DOPI_DevObj_t* t_obj, uint8_t* t_byteArr)
{
    int t_cursor = 0;
    uint32_t t_data = 0;

    memcpy(&t_data, t_byteArr, 4);
    t_obj->noti=t_data;
...
//    uint16_t t_identifier = MANU|(NODE_ID_CM<<4)|t_dest_node;
//    uint16_t t_checksum;
//
//    t_tx_data[0] = 7;// TASK_ID_MANU
...
//    }
//
//    t_res = Checksum_Check(t_byte_arr, 62, &t_byte_arr[62]);
//
...
static int ReadSDO(DOPI_DevObj_t* t_obj, uint8_t* t_byte_arr)
{
    int t_byte_read = 0;
...
static int ReadBLESDO(DOPI_DevObj_t* t_obj, uint8_t* t_byteArr, uint32_t buf_len, uint32_t* current_index, uint8_t t_taskID)
{
    uint16_t t_SDOID;
    uint8_t t_numOfData = 1;
    int t_size = 0;

    // SDO ID 읽기
    t_SDOID = t_byteArr[*current_index];
    *current_index = (*current_index + 1) % buf_len; // 인덱스를 순환 버퍼에 맞게 업데이트

    // // SDO ID : Neutral Posture Offset인 경우, flag on
    // if (t_SDOID == SDO_ID_ANGELA_CTM_RIGHT_THIGH_NEUTRAL_SAGITTAL_OFFSET) {
    //     rightHipNPOffsetFlag = true;  // Offset Cal ON
    // } else if (t_SDOID == SDO_ID_ANGELA_CTM_LEFT_THIGH_NEUTRAL_SAGITTAL_OFFSET) {
    //     leftHipNPOffsetFlag = true;   // Offset Cal ON
    // } else if (t_SDOID == SDO_ID_ANGELA_CTM_LEFT_SHANK_NEUTRAL_SAGITTAL_OFFSET) {
    //     rightKneeNPOffsetFlag = true;  // Offset Cal ON
    // } else if (t_SDOID == SDO_ID_ANGELA_CTM_RIGHT_SHANK_NEUTRAL_SAGITTAL_OFFSET) {
    //     leftKneeNPOffsetFlag = true;   // Offset Cal ON
    // }

    // 데이터 유형 및 크기 계산
    uint8_t t_dataType = SDOTable[t_taskID][t_SDOID];
    t_size = t_numOfData * DOP_ConvertDataSize(t_dataType);

    // 임시 버퍼를 사용하여 데이터를 복사
    uint8_t temp_buffer[t_size];

    // 데이터를 복사할 때 링 버퍼의 경계를 넘어가는 경우를 처리
    if ((*current_index + t_size) <= buf_len) { // 데이터가 경계를 넘지 않는 경우
        memcpy(temp_buffer, &t_byteArr[*current_index], t_size);
        *current_index = (*current_index + t_size) % buf_len;
    } else { // 데이터가 경계를 넘는 경우
        uint32_t first_part_size = buf_len - *current_index;
        memcpy(temp_buffer, &t_byteArr[*current_index], first_part_size);
        memcpy(temp_buffer + first_part_size, &t_byteArr[0], t_size - first_part_size);
        *current_index = (t_size - first_part_size) % buf_len;
    }

    // 최종 데이터 복사
    memcpy(t_obj->tasks[t_taskID].SDOsAddr[t_SDOID], &temp_buffer, t_size);
...
// {
//     uint8_t t_cursor = 0;
//     uint8_t t_taskID = TASK_ID_ANGELA;
//     uint8_t t_numOfData = 1;
//     uint16_t t_SDOID;
//     // int8_t t_SDOStatus;
...
//     uint8_t t_size = DOP_ConvertDataSize(SDOTable[t_taskID][t_SDOID]);
//     memcpy(t_obj->tasks[t_taskID].SDOsAddr[t_SDOID], &t_byteArr[t_cursor], t_size);
//     t_cursor += t_size;
