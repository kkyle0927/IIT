...
/**
 * @brief Find the GPIO handle for a given GPIO port.
 * @param gpioPort The GPIO port identifier.
 * @return GPIO_TypeDef* The handle of the specified GPIO port, or NULL if not found.
 */
GPIO_TypeDef* BSP_GPIO_FindHandle(BSP_GPIOPort_t gpioPort)
{
    GPIO_TypeDef* handle = FindGPIOHandle(gpioPort);
