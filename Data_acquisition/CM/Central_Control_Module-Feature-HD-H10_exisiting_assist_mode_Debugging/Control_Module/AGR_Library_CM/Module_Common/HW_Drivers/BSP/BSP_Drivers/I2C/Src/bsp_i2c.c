 ...
 * @return HAL status.
 */
BSP_StatusTypeDef_t BSP_I2C_Init(BSP_I2C_t i2c)
{
	BSP_StatusTypeDef_t ret = BSP_OK;
    I2C_HandleTypeDef* hi2c = FindI2CHandle(i2c);

    ret = (BSP_StatusTypeDef_t)HAL_I2C_Init(hi2c);
    HAL_I2C_MspInit(hi2c);

    return ret;
}

void BSP_I2C_MspInit(BSP_I2C_t i2c)
{
    I2C_HandleTypeDef* hi2c = FindI2CHandle(i2c);
 ...
 * @return HAL status.
 */
BSP_StatusTypeDef_t BSP_I2C_DeInit(BSP_I2C_t i2c)
{
	BSP_StatusTypeDef_t ret = BSP_OK;
    I2C_HandleTypeDef* hi2c = FindI2CHandle(i2c);

    ret = (BSP_StatusTypeDef_t)HAL_I2C_DeInit(hi2c);
    HAL_I2C_MspDeInit(hi2c);

    return ret;
}

void BSP_I2C_MspDeInit(BSP_I2C_t i2c)
{
    I2C_HandleTypeDef* hi2c = FindI2CHandle(i2c);
...
/**
 * @brief Rest the I2C bus.
 * @param i2c Identifier for the I2C peripheral.
 * @return HAL status.
 */
void BSP_I2C_BusReset(BSP_I2C_t i2c, BSP_GPIOPort_t SDAgpioPort, uint16_t SDAgpioPin, BSP_GPIOPort_t SCLgpioPort, uint16_t SCLgpioPin)
{
    I2C_HandleTypeDef* hi2c = FindI2CHandle(i2c);
    GPIO_TypeDef* GPIOxSDA  = BSP_GPIO_FindHandle(SDAgpioPort);
    GPIO_TypeDef* GPIOxSCL  = BSP_GPIO_FindHandle(SCLgpioPort);

    HAL_I2C_BusReset(hi2c, GPIOxSDA, SDAgpioPin, GPIOxSCL, SCLgpioPin);
}

/**
 * @brief Find the I2C handle for a given I2C port.
 * @param i2c The I2C port identifier.
 * @return GPIO_TypeDef* The handle of the specified I2C port, or NULL if not found.
 */
I2C_HandleTypeDef* BSP_I2C_FindHandle(BSP_I2C_t i2cPort)
{
	I2C_HandleTypeDef* handle = FindI2CHandle(i2cPort);
...
BSP_StatusTypeDef_t BSP_I2CMasterDMA(BSP_I2C_t i2c, uint16_t devAddress, uint8_t* pData, uint16_t size, BSP_I2COP_t operation)
{
    ...
    //ble 사용 시 Hardfault 발생 in SUIT
#if defined(WALKON5_CM_ENABLED) || defined(L30_CM_ENABLED) && defined (_USE_SEMAPHORE)
    BSP_I2CSemMap_t BinSemI2C = FindI2CSemaphoreHandle(i2c);
