...
uint32_t BSP_GetBaudRateUART(BSP_UART_t uart)
{
	uint32_t baudrate = 0;
 ...
 */
void BSP_SetUARTCB(BSP_UART_t uart, BSP_UARTCBType_t callbackType, BSP_UARTCBPtr_t callback, void* params)
{
    if (uart < BSP_UART_COUNT && callbackType < BSP_UART_CALLBACK_TYPE_COUNT && callback) {
        bsp_uartCB[uart].callbacks[callbackType] = callback;
        bsp_uartCB[uart].params[callbackType] = params;
    }
}
...
static void ExecuteUARTCB(UART_HandleTypeDef* huart, BSP_UARTCBType_t callbackType)
{
    ...
    if (uart < BSP_UART_COUNT) {
        ...
        void* params = bsp_uartCB[uart].params[callbackType];
        if (callback) {
