...
volatile bool IsUSBDeviceDet = false;
...
static IOIF_USB_Tim_t usb_timer_source = IOIF_USB_TIM_NULL;
...
//		IOIF_SetGPIOCB(MCU_USB_FS_VBUS_Pin, IOIF_GPIO_EXTI_CALLBACK, USBDVBUSDetCB);
	...
	case IOIF_USBD_MSC:
		IOIF_SetUSBDetCB();			// Set USB Device Vbus Detection Callback
		if(BSP_USBInit(BSP_USBD_MSC) != true)
			ret = IOIF_USB_FAIL;
		usb_mode = IOIF_USBD_MSC;
...
IOIF_USBState IOIF_DeinitUSBCDC(void)
{
	IOIF_USBState ret = IOIF_USB_OK;

	if(BSP_USBCDCDeinit() == false)
		ret = IOIF_USB_FAIL;
